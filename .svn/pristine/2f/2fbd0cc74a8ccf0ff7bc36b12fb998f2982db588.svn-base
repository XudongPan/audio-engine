...
AoECodec* AoECodec::GetInterface(AudioEngine* voiceEngine)
{
#ifndef VOIP_AUDIO_ENGINE_CODEC_API
    return NULL;
#else
    if (NULL == voiceEngine)
    {
...
int VoECodecImpl::GetCodec(int index, CodecInst& codec)
{
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
        VoEId(_shared->instance_id(), -1),
        "GetCodec() => plname=%s, packsize=%d, plfreq=%d, pltype=%d, "
        "channels=%d, rate=%d", codec.plname, codec.packsize,
...
int VoECodecImpl::SetSendCodec(int channel, const CodecInst& codec)
{
    ...
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_shared->instance_id(), -1),
                 "codec: plname=%s, packsize=%d, plfreq=%d, pltype=%d, "
                 "channels=%d, rate=%d", codec.plname, codec.packsize,
                 codec.plfreq, codec.pltype, codec.channels, codec.rate);
...
int VoECodecImpl::GetSendCodec(int channel, CodecInst& codec)
{
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
        VoEId(_shared->instance_id(), -1),
        "GetSendCodec() => plname=%s, packsize=%d, plfreq=%d, "
        "channels=%d, rate=%d", codec.plname, codec.packsize,
...
int VoECodecImpl::GetRecCodec(int channel, CodecInst& codec)
{
    ...
    WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
        VoEId(_shared->instance_id(), -1),
        "GetRecCodec() => plname=%s, packsize=%d, plfreq=%d, "
        "channels=%d, rate=%d", codec.plname, codec.packsize,
        codec.plfreq, codec.channels, codec.rate);
    return 0;
}
...
int VoECodecImpl::SetRecPayloadType(int channel, const CodecInst& codec)
{
    ...
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_shared->instance_id(), -1),
               ...
               "packsize=%d, rate=%d", codec.plname, codec.plfreq, codec.pltype,
               codec.channels, codec.packsize, codec.rate);
    if (!_shared->statistics().Initialized())
    {
        _shared->SetLastError(VE_NOT_INITED, kTraceError);
        return -1;
                                                    ...
                                                    const CodecInst& fromInst)
{
    ...
    if (STR_CASE_CMP(fromInst.plname,"SILK") == 0)
    {
        ...
        {
            if (fromInst.packsize == 320)
            {
                toInst.packsize = 240;
            }
            else if (fromInst.packsize == 640)
            {
                toInst.packsize = 480;
            }
            else if (fromInst.packsize == 960)
            {
                toInst.packsize = 720;
            }
        }
        ...
        {
            if (fromInst.packsize == 640)
            {
                toInst.packsize = 480;
            }
            else if (fromInst.packsize == 1280)
            {
                toInst.packsize = 960;
            }
            else if (fromInst.packsize == 1920)
            {
                toInst.packsize = 1440;
            }
        }
    }
}
                                                    ...
                                                    const CodecInst& fromInst)
{
    ...
    if (STR_CASE_CMP(fromInst.plname,"SILK") == 0)
    {
        if (fromInst.plfreq == 12000)
        {
            if (fromInst.packsize == 240)
            {
                toInst.packsize = 320;
            }
            else if (fromInst.packsize == 480)
            {
                toInst.packsize = 640;
            }
            else if (fromInst.packsize == 720)
            {
                toInst.packsize = 960;
            }
        }
        ...
        {
            if (fromInst.packsize == 480)
            {
                toInst.packsize = 640;
            }
            else if (fromInst.packsize == 960)
            {
                toInst.packsize = 1280;
            }
            else if (fromInst.packsize == 1440)
            {
                toInst.packsize = 1920;
            }
        }
    }
}
...
int VoECodecImpl::GetSecondarySendCodec(int channel, CodecInst& codec) {
  ...
  WEBRTC_TRACE(kTraceStateInfo, kTraceVoice,
               VoEId(_shared->instance_id(), -1),
               "GetSecondarySendCodec() => plname=%s, packsize=%d, plfreq=%d, "
               "channels=%d, rate=%d", codec.plname, codec.packsize,
               codec.plfreq, codec.channels, codec.rate);
  return 0;
}
...
#endif  // VOIP_AUDIO_ENGINE_CODEC_API
