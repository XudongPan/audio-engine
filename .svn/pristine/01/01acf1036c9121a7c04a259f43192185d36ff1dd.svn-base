...
AoEBase* AoEBase::GetInterface(AudioEngine* audioEngine)
{
    if (NULL == audioEngine)
    {
        return NULL;
...
AoEBaseImpl::AoEBaseImpl(aoe::SharedData* shared) :
...
void AoEBaseImpl::OnErrorIsReported(ErrorCode error)
{
        ...
        if (_voiceEngineObserverPtr)
        {
            ...
            if (error == AudioDeviceObserver::kRecordingError)
            {
                ...
                VOIP_TRACE(kTraceInfo, kTraceVoice,
                    ...
                    "AoEBaseImpl::OnErrorIsReported() => VE_RUNTIME_REC_ERROR");
            }
...
void AoEBaseImpl::OnWarningIsReported(WarningCode warning)
{
    CriticalSectionScoped cs(&_callbackCritSect);
    if (_voiceEngineObserver)
    {
...
int32_t AoEBaseImpl::RecordedDataIsAvailable(
        ...
        uint32_t& newMicLevel)
{
    VOIP_TRACE(kTraceStream, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "AoEBaseImpl::RecordedDataIsAvailable(nSamples=%u, "
...
int32_t AoEBaseImpl::NeedMorePlayData(
        uint32_t nSamples,
        uint8_t nBytesPerSample,
        uint8_t nChannels,
        uint32_t samplesPerSec,
...
{
  VOIP_TRACE(kTraceStream, kTraceVoice, AoEId(_shared->instance_id(), -1),
               "AoEBaseImpl::NeedMorePlayData(nSamples=%u, "
...
int AoEBaseImpl::OnDataAvailable(const int voe_channels[],
  ...
  VOIP_TRACE(kTraceStream, kTraceVoice, AoEId(_shared->instance_id(), -1),
               "AoEBaseImpl::OnDataAvailable(number_of_voe_channels=%d, "
  ...
  // No need to go through the APM, demultiplex the data to each AoE channel,
...
void AoEBaseImpl::OnData(int voe_channel, const void* audio_data,
...
void AoEBaseImpl::PushCaptureData(int voe_channel, const void* audio_data,
                                  int bits_per_sample, int sample_rate,
                                  int number_of_channels,
                                  int number_of_frames) {
  aoe::ChannelOwner ch = _shared->channel_manager().GetChannel(voe_channel);
  aoe::Channel* channel_ptr = ch.channel();
  if (!channel_ptr)
    return;
...
void AoEBaseImpl::PullRenderData(int bits_per_sample, int sample_rate,
...
int AoEBaseImpl::RegisterAudioEngineObserver(AudioEngineObserver& observer)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "RegisterAudioEngineObserver(observer=0x%d)", &observer);
    CriticalSectionScoped cs(&_callbackCritSect);
    if (_voiceEngineObserverPtr)
    {
    ...
    for (aoe::ChannelManager::Iterator it(&_shared->channel_manager());
         it.IsValid();
         it.Increment()) {
      it.GetChannel()->RegisterAudioEngineObserver(observer);
    }

    _shared->transmit_mixer()->RegisterAudioEngineObserver(observer);
...
int AoEBaseImpl::DeRegisterAudioEngineObserver()
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "DeRegisterAudioEngineObserver()");
    CriticalSectionScoped cs(&_callbackCritSect);
    if (!_voiceEngineObserverPtr)
    {
        _shared->SetLastError(VE_INVALID_OPERATION, kTraceError,
    ...
    for (aoe::ChannelManager::Iterator it(&_shared->channel_manager());
         it.IsValid();
         it.Increment()) {
      it.GetChannel()->DeRegisterAudioEngineObserver();
    }

    return 0;
}

int AoEBaseImpl::Init(AudioDeviceModule* external_adm,
                      AudioProcessing* audioproc)
{
    ...
    if (external_adm == NULL)
    {
        ...
        _shared->set_audio_device(AudioDeviceModuleImpl::Create(
            AoEId(_shared->instance_id(), -1), _shared->audio_device_layer()));
    ...
    else
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_shared->instance_id(), -1),
            "An external ADM implementation will be used in AudioEngine");
    }
    ...
    if (_shared->audio_device()->SetPlayoutDevice(
            VOIP_AUDIO_ENGINE_DEFAULT_DEVICE) != 0)
    {
    ...
    // http://code.google.com/p/VoIP/issues/detail?id=204
    ...
    if (!audioproc) {
      audioproc = AudioProcessing::Create(AoEId(_shared->instance_id(), -1));
      if (!audioproc) {
    ...
    if (_shared->audio_device()->SetAGC(agc_enabled) != 0) {
      ...
      // https://code.google.com/p/VoIP/issues/detail?id=1464
    }
#endif
...
int AoEBaseImpl::CreateChannel() {
  ...
  aoe::ChannelOwner channel_owner = _shared->channel_manager().CreateChannel();

  return InitializeChannel(&channel_owner);
}

int AoEBaseImpl::CreateChannel(const Config& config) {
  CriticalSectionScoped cs(_shared->crit_sec());
  if (!_shared->statistics().Initialized()) {
      _shared->SetLastError(VE_NOT_INITED, kTraceError);
      return -1;
...
int AoEBaseImpl::InitializeChannel(aoe::ChannelOwner* channel_owner)
{
...
int AoEBaseImpl::DeleteChannel(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StartReceive(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StopReceive(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StartPlayout(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StopPlayout(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StartSend(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::StopSend(int channel)
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
...
int AoEBaseImpl::GetVersion(char version[1024])
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "GetVersion(version=?)");
    assert(kAudioEngineVersionMaxMessageSize == 1024);
    ...
    char versionBuf[kAudioEngineVersionMaxMessageSize];
    char* versionPtr = versionBuf;

    int32_t len = 0;
    int32_t accLen = 0;

    len = AddAoEVersion(versionPtr);
    if (len == -1)
    {
        return -1;
    }
    versionPtr += len;
    accLen += len;
    assert(accLen < kAudioEngineVersionMaxMessageSize);
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceVoice,
        AoEId(_shared->instance_id(), -1), "GetVersion() =>");
    for (int partStart = 0; partStart < accLen;)
    {
        ...
        partStart = partEnd;
        VOIP_TRACE(kTraceStateInfo, kTraceVoice,
            AoEId(_shared->instance_id(), -1), "%s", partOfVersion);
    }

    return 0;
}

int32_t AoEBaseImpl::AddBuildInfo(char* str) const
{
    return sprintf(str, "Build: %s\n", BUILDINFO);
}

int32_t AoEBaseImpl::AddAoEVersion(char* str) const
{
    return sprintf(str, "AudioEngine 1.1.0\n");
}

#ifdef VOIP_EXTERNAL_TRANSPORT
int32_t AoEBaseImpl::AddExternalTransportBuild(char* str) const
{
...
#ifdef VOIP_AOE_EXTERNAL_REC_AND_PLAYOUT
int32_t AoEBaseImpl::AddExternalRecAndPlayoutBuild(char* str) const
{
    return sprintf(str, "External recording and playout build\n");
}
#endif

int AoEBaseImpl::LastError()
{
    VOIP_TRACE(kTraceApiCall, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "LastError()");
...
int32_t AoEBaseImpl::StartPlayout()
{
    VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "AoEBaseImpl::StartPlayout()");
...
int32_t AoEBaseImpl::StopPlayout() {
  VOIP_TRACE(kTraceInfo,
               kTraceVoice,
               AoEId(_shared->instance_id(), -1),
               "AoEBaseImpl::StopPlayout()");
...
int32_t AoEBaseImpl::StartSend()
{
    VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "AoEBaseImpl::StartSend()");
...
int32_t AoEBaseImpl::StopSend()
{
    VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "AoEBaseImpl::StopSend()");
...
int32_t AoEBaseImpl::TerminateInternal()
{
    VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_shared->instance_id(), -1),
                 "AoEBaseImpl::TerminateInternal()");
...
int AoEBaseImpl::ProcessRecordedDataWithAPM(
  ...
  if (volume != 0) {
    // Scale from ADM to AoE level range
  ...
  // do the operations on all the existing AoE channels; otherwise the
  ...
  // Scale from AoE to ADM level range.
...
void AoEBaseImpl::GetPlayoutData(int sample_rate, int number_of_channels,
...
int AoEBaseImpl::Start( int channel )
{
	int ret = 0;
	
	ret &= this->StartSend(channel);
	ret &= this->StartReceive(channel);
	ret &= this->StartPlayout(channel);

	return ret;
}

int AoEBaseImpl::Stop( int channel )
{
	int ret = 0;

	ret &= this->StopPlayout(channel);
	

	ret &= this->StopSend(channel);
	

	ret &= this->StopReceive(channel);
