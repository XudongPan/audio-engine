...
AoEIO* AoEIO::GetInterface(AudioEngine* voiceEngine)
{
    if (NULL == voiceEngine)
    {
        return NULL;
...
int VoENetworkImpl::ReceivedPacket(int channel,
                                      const void* data,
                                      unsigned int length) {
  return ReceivedPacket(channel, data, length, VoIP::PacketTime());
}
