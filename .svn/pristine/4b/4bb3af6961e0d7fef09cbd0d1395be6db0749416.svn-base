...
#include "audio_engine/modules/audio_package/source/audio_ctrl_pkg_sender.h"
...
int RTCPSender::PrepareRTCP(const FeedbackState& feedback_state,
  ...
  {
      rtcpPacketTypeFlags |= kRtcpXrwebrtcMetric;
      _xrSendVoIPMetric = false;
  }
  ...
  if(rtcpPacketTypeFlags & kRtcpXrwebrtcMetric)
  {
                          ...
                          const uint16_t length)
{
    ...
    if(_cbTransport)
    {
        if(_cbTransport->SendCtrlPacket(_id, dataBuffer, length) > 0)
        {
            return 0;
        }
    }
...
int32_t
RTCPSender::SetCSRCs(const uint32_t arrOfCSRC[kCsrcSize],
                    const uint8_t arrLength)
{
    assert(arrLength <= kCsrcSize);
    CriticalSectionScoped lock(_criticalSectionRTCPSender);
...
int32_t
RTCPSender::SetRTCPVoIPMetrics(const RTCPVoIPMetric* webrtcMetric)
{
    CriticalSectionScoped lock(_criticalSectionRTCPSender);
    memcpy(&_xrVoIPMetric, webrtcMetric, sizeof(RTCPVoIPMetric));
