...
#include <stdio.h>
#endif
#include "audio_engine/modules/audio_processing/agc/analog_agc.h"
...
int VoipAgc_AddMic(void *state, int16_t *in_mic, int16_t *in_mic_H,
                     int16_t samples)
{
    ...
    if (stt->micVol > stt->maxAnalog)
    {
        ...
        tmp32 = VOIP_SPL_MUL_16_16(GAIN_TBL_LEN - 1, tmp16);
        tmp16 = (int16_t)(stt->maxLevel - stt->maxAnalog);
        targetGainIdx = (uint16_t)VOIP_SPL_DIV(tmp32, tmp16);
        ...
        {
            // For lower band
            tmp32 = VOIP_SPL_MUL_16_U16(in_mic[i], gain);
            sample = VOIP_SPL_RSHIFT_W32(tmp32, 12);
            if (sample > 32767)
            {
                in_mic[i] = 32767;
            ...
            if (stt->fs == 32000)
            {
                tmp32 = VOIP_SPL_MUL_16_U16(in_mic_H[i], gain);
                sample = VOIP_SPL_RSHIFT_W32(tmp32, 12);
                if (sample > 32767)
                {
                    in_mic_H[i] = 32767;
    ...
    for (i = 0; i < M; i++)
    {
        ...
        for (n = 0; n < L; n++)
        {
            nrg = VOIP_SPL_MUL_16_16(in_mic[i * L + n], in_mic[i * L + n]);
            if (nrg > max_nrg)
            {
                max_nrg = nrg;
            }
    ...
    for (i = 0; i < VOIP_SPL_RSHIFT_W16(M, 1); i++)
    {
        if (stt->fs == 16000)
        {
            VoipSpl_DownsampleBy2(&in_mic[i * 32], 32, tmp_speech, stt->filterState);
        } else
        {
            memcpy(tmp_speech, &in_mic[i * 16], 16 * sizeof(short));
        }
        /* Compute energy in blocks of 16 samples */
        ptr[i] = VoipSpl_DotProductWithScale(tmp_speech, tmp_speech, 16, 4);
    }
    ...
    {
        VoipAgc_ProcessVad(&stt->vadMic, &in_mic[i], subFrames);
    }

    return 0;
}

int VoipAgc_AddFarend(void *state, const int16_t *in_far, int16_t samples)
{
    int32_t errHandle = 0;
    int16_t i, subFrames;
    Agc_t *stt;
    ...
    for (i = 0; i < samples; i += subFrames)
    {
        errHandle += VoipAgc_AddFarendToDigital(&stt->digitalAgc, &in_far[i], subFrames);
    }

    return errHandle;
}

int VoipAgc_VirtualMic(void *agcInst, int16_t *in_near, int16_t *in_near_H,
                         ...
                         int32_t *micLevelOut)
{
    ...
    frameNrg = VOIP_SPL_MUL_16_16(in_near[0], in_near[0]);
    for (sampleCntr = 1; sampleCntr < samples; sampleCntr++)
    {
        ...
        {
            nrg = VOIP_SPL_MUL_16_16(in_near[sampleCntr], in_near[sampleCntr]);
            frameNrg += nrg;
        }
    ...
    micLevelTmp = VOIP_SPL_LSHIFT_W32(micLevelIn, stt->scale);
    /* Set desired level */
    gainIdx = stt->micVol;
    if (stt->micVol > stt->maxAnalog)
    {
    ...
    {
        tmpFlt = VOIP_SPL_RSHIFT_W32(VOIP_SPL_MUL_16_U16(in_near[ii], gain), 10);
        if (tmpFlt > 32767)
        {
            tmpFlt = 32767;
            gainIdx--;
        ...
        if (stt->fs == 32000)
        {
            tmpFlt = VOIP_SPL_MUL_16_U16(in_near_H[ii], gain);
            tmpFlt = VOIP_SPL_RSHIFT_W32(tmpFlt, 10);
            if (tmpFlt > 32767)
            {
                tmpFlt = 32767;
            }
...
//    *micLevelOut = stt->micGainIdx;
    *micLevelOut = VOIP_SPL_RSHIFT_W32(stt->micGainIdx, stt->scale);
    /* Add to Mic as if it was the output from a true microphone */
    if (VoipAgc_AddMic(agcInst, in_near, in_near_H, samples) != 0)
    {
        return -1;
    }
    return 0;
...
void VoipAgc_UpdateAgcThresholds(Agc_t *stt)
{
    ...
    if (stt->micLvlSat)
    {
        ...
        zeros = VoipSpl_NormW32(stt->Rxx160_LPw32);
        stt->targetIdxOffset = VOIP_SPL_RSHIFT_W16((3 * zeros) - stt->targetIdx - 2, 2);
    }
#endif

    /* Set analog target level in envelope dBOv scale */
    tmp16 = (DIFF_REF_TO_ANALOG * stt->compressionGaindB) + ANALOG_TARGET_LEVEL_2;
    tmp16 = VoipSpl_DivW32W16ResW16((int32_t)tmp16, ANALOG_TARGET_LEVEL);
...
void VoipAgc_SaturationCtrl(Agc_t *stt, uint8_t *saturated, int32_t *env)
{
    int16_t i, tmpW16;
    ...
    for (i = 0; i < 10; i++)
    {
        tmpW16 = (int16_t)VOIP_SPL_RSHIFT_W32(env[i], 20);
        if (tmpW16 > 875)
        {
            stt->envSum += tmpW16;
        }
    ...
    /* stt->envSum *= 0.99; */
    stt->envSum = (int16_t)VOIP_SPL_MUL_16_16_RSFT(stt->envSum,
            (int16_t)32440, 15);
}

void VoipAgc_ZeroCtrl(Agc_t *stt, int32_t *inMicLevel, int32_t *env)
{
    int16_t i;
    int32_t tmp32 = 0;
    int32_t midVal;
    ...
    if (stt->msZero > 500)
    {
        ...
        midVal = VOIP_SPL_RSHIFT_W32(stt->maxAnalog + stt->minLevel + 1, 1);
        if (*inMicLevel < midVal)
        {
            /* *inMicLevel *= 1.1; */
            tmp32 = VOIP_SPL_MUL(1126, *inMicLevel);
            *inMicLevel = VOIP_SPL_RSHIFT_W32(tmp32, 10);
            /* Reduces risk of a muted mic repeatedly triggering excessive levels due
             * to zero signal detection. */
            *inMicLevel = VOIP_SPL_MIN(*inMicLevel, stt->zeroCtrlMax);
...
void VoipAgc_SpeakerInactiveCtrl(Agc_t *stt)
{
    ...
    } else
    {
        ...
        if (stt->vadMic.stdLongTerm < 4500)
        {
            ...
            vadThresh += VOIP_SPL_RSHIFT_W16(4500 - stt->vadMic.stdLongTerm, 1);
        }

        /* stt->vadThreshold = (31 * stt->vadThreshold + vadThresh) / 32; */
        tmp32 = (int32_t)vadThresh;
        tmp32 += VOIP_SPL_MUL_16_16((int16_t)31, stt->vadThreshold);
        stt->vadThreshold = (int16_t)VOIP_SPL_RSHIFT_W32(tmp32, 5);
    }
}

void VoipAgc_ExpCurve(int16_t volume, int16_t *index)
{
    // volume in Q14
    // index in [0-7]
    /* 8 different curves */
...
int32_t VoipAgc_ProcessAnalog(void *state, int32_t inMicLevel,
                                int32_t *outMicLevel,
                                int16_t vadLogRatio,
                                int16_t echo, uint8_t *saturationWarning)
{
    ...
    stt = (Agc_t *)state;
    inMicLevelTmp = VOIP_SPL_LSHIFT_W32(inMicLevel, stt->scale);
    ...
    if (stt->firstCall == 0)
    {
        int32_t tmpVol;
        stt->firstCall = 1;
        tmp32 = VOIP_SPL_RSHIFT_W32((stt->maxLevel - stt->minLevel) * (int32_t)51, 9);
     ...
     */
    VoipAgc_SaturationCtrl(stt, &saturated, stt->env[0]);
    ...
    if (saturated == 1)
    {
        ...
        stt->Rxx160_LPw32 = VOIP_SPL_MUL(VOIP_SPL_RSHIFT_W32(stt->Rxx160_LPw32, 3), 7);

        stt->zeroCtrlMax = stt->micVol;

        /* stt->micVol *= 0.903; */
        tmp32 = inMicLevelTmp - stt->minLevel;
        tmpU32 = VOIP_SPL_UMUL(29591, (uint32_t)(tmp32));
        stt->micVol = (int32_t)VOIP_SPL_RSHIFT_U32(tmpU32, 15) + stt->minLevel;
     ...
     * level as 17% */
    VoipAgc_ZeroCtrl(stt, &inMicLevelTmp, stt->env[0]);
     ...
     * silence.
     */
    VoipAgc_SpeakerInactiveCtrl(stt);
    ...
    for (i = 0; i < 5; i++)
    {
        ...
        /* Rxx160w32 in Q(-7) */
        tmp32 = VOIP_SPL_RSHIFT_W32(Rxx16w32 - stt->Rxx16_vectorw32[stt->Rxx16pos], 3);
        ...
        /* Rxx16_LPw32 in Q(-4) */
        tmp32 = VOIP_SPL_RSHIFT_W32(Rxx16w32 - stt->Rxx16_LPw32, kAlphaShortTerm);
        ...
        if (vadLogRatio > stt->vadThreshold)
        {
            ...
            {
                stt->activeSpeech += 2;
                tmp32 = VOIP_SPL_RSHIFT_W32(stt->Rxx16_LPw32Max, 3);
                stt->Rxx160_LPw32 = VOIP_SPL_MUL(tmp32, RXX_BUFFER_LEN);
            }

            tmp32 = VOIP_SPL_RSHIFT_W32(stt->Rxx160w32 - stt->Rxx160_LPw32, kAlphaLongTerm);
            ...
            if (stt->Rxx160_LPw32 > stt->upperSecondaryLimit)
            {
                ...
                if (stt->msTooHigh > stt->msecSpeechOuterChange)
                {
                    ...
                    tmp32 = VOIP_SPL_RSHIFT_W32(stt->Rxx160_LPw32, 6);
                    stt->Rxx160_LPw32 = VOIP_SPL_MUL(tmp32, 53);
                    ...
                    stt->maxLevel = VOIP_SPL_RSHIFT_W32(tmp32, 4);
                    stt->maxLevel = VOIP_SPL_MAX(stt->maxLevel, stt->maxAnalog);

                    stt->zeroCtrlMax = stt->micVol;

                    /* 0.95 in Q15 */
                    tmp32 = inMicLevelTmp - stt->minLevel;
                    tmpU32 = VOIP_SPL_UMUL(31130, (uint32_t)(tmp32));
                    stt->micVol = (int32_t)VOIP_SPL_RSHIFT_U32(tmpU32, 15) + stt->minLevel;
            ...
            } else if (stt->Rxx160_LPw32 > stt->upperLimit)
            {
                ...
                if (stt->msTooHigh > stt->msecSpeechInnerChange)
                {
                    ...
                    /* 0.965 in Q15 */
                    tmp32 = inMicLevelTmp - stt->minLevel;
                    tmpU32 = VOIP_SPL_UMUL(31621, (uint32_t)(inMicLevelTmp - stt->minLevel));
                    stt->micVol = (int32_t)VOIP_SPL_RSHIFT_U32(tmpU32, 15) + stt->minLevel;
                    if (stt->micVol > lastMicVol - 1)
                    {
                        stt->micVol = lastMicVol - 1;
            ...
            } else if (stt->Rxx160_LPw32 < stt->lowerSecondaryLimit)
            {
                ...
                if (stt->msTooLow > stt->msecSpeechOuterChange)
                {
                    ...
                    tmp32 = VOIP_SPL_LSHIFT_W32(inMicLevelTmp - stt->minLevel, 14);
                    if (stt->maxInit != stt->minLevel)
                    {
                        volNormFIX = (int16_t)VOIP_SPL_DIV(tmp32,
                                                              (stt->maxInit - stt->minLevel));
                    }

                    /* Find correct curve */
                    VoipAgc_ExpCurve(volNormFIX, &index);

                    /* Compute weighting factor for the volume increase, 32^(-2*X)/2+1.05 */
                    weightFIX = kOffset1[index]
                              - (int16_t)VOIP_SPL_MUL_16_16_RSFT(kSlope1[index],
                                                                         volNormFIX, 13);

                    /* stt->Rxx160_LPw32 *= 1.047 [~0.2 dB]; */
                    tmp32 = VOIP_SPL_RSHIFT_W32(stt->Rxx160_LPw32, 6);
                    stt->Rxx160_LPw32 = VOIP_SPL_MUL(tmp32, 67);

                    tmp32 = inMicLevelTmp - stt->minLevel;
                    tmpU32 = ((uint32_t)weightFIX * (uint32_t)(inMicLevelTmp - stt->minLevel));
                    stt->micVol = (int32_t)VOIP_SPL_RSHIFT_U32(tmpU32, 14) + stt->minLevel;
                    if (stt->micVol < lastMicVol + 2)
                    {
                        stt->micVol = lastMicVol + 2;
                    }
            ...
            } else if (stt->Rxx160_LPw32 < stt->lowerLimit)
            {
                ...
                if (stt->msTooLow > stt->msecSpeechInnerChange)
                {
                    ...
                    weightFIX = kOffset2[index]
                              - (int16_t)VOIP_SPL_MUL_16_16_RSFT(kSlope2[index],
                                                                         volNormFIX, 13);
            ...
            if (stt->numBlocksMicLvlSat > NUM_BLOCKS_IN_SAT_BEFORE_CHANGE_TARGET)
            {
                ...
                VoipAgc_UpdateAgcThresholds(stt);
                VoipAgc_CalculateGainTable(&(stt->digitalAgc.gainTable[0]),
    ...
    *outMicLevel = VOIP_SPL_RSHIFT_W32(stt->micVol, stt->scale);
    if (*outMicLevel > VOIP_SPL_RSHIFT_W32(stt->maxAnalog, stt->scale))
    {
        *outMicLevel = VOIP_SPL_RSHIFT_W32(stt->maxAnalog, stt->scale);
    }

    return 0;
}

int VoipAgc_Process(void *agcInst, const int16_t *in_near,
                      ...
                      uint8_t *saturationWarning)
{
    ...
    *outMicLevel = inMicLevel;
    ...
    for (i = 0; i < samples; i += subFrames)
    {
        if (VoipAgc_ProcessDigital(&stt->digitalAgc, &in_near[i], &in_near_H[i], &out[i], &out_H[i],
        ...
        {
            if (VoipAgc_ProcessAnalog(agcInst, inMicLevelTmp, outMicLevel,
...
int VoipAgc_set_config(void *agcInst, VoipAgc_config_t agcConfig)
{
    Agc_t *stt;
    stt = (Agc_t *)agcInst;
    ...
    if (VoipAgc_CalculateGainTable(&(stt->digitalAgc.gainTable[0]), stt->compressionGaindB,
    ...
    /* Store the config in a VoIPAgc_config_t */
...
int VoipAgc_get_config(void *agcInst, VoipAgc_config_t *config)
{
    Agc_t *stt;
    stt = (Agc_t *)agcInst;
...
int VoipAgc_Create(void **agcInst)
{
    Agc_t *stt;
    if (agcInst == NULL)
    {
 ...
 */
int VoipAgc_Init(void *agcInst, int32_t minLevel, int32_t maxLevel,
                   int16_t agcMode, uint32_t fs)
{
    ...
    if (VoipAgc_InitDigital(&stt->digitalAgc, agcMode) != 0)
    {
        stt->lastError = AGC_UNINITIALIZED_ERROR;
        return -1;
    }
    ...
    stt->scale = 0;
    maxLevel = VOIP_SPL_LSHIFT_W32(maxLevel, stt->scale);
    minLevel = VOIP_SPL_LSHIFT_W32(minLevel, stt->scale);
    ...
    max_add = VOIP_SPL_RSHIFT_W32(maxLevel - minLevel, 2);
    ...
    tmp32 = VOIP_SPL_RSHIFT_W32((stt->maxLevel - stt->minLevel) * (int32_t)10, 8);
    ...
    VoipSpl_MemSetW32(stt->filterState, 0, 8);
    ...
    if (VoipAgc_set_config(stt, stt->defaultConfig) == -1)
    {
