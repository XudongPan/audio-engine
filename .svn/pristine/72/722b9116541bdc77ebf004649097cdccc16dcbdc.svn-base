...
{
    VOIP_TRACE(kTraceMemory, kTraceVoice, AoEId(instanceId, -1),
...
int32_t
TransmitMixer::RegisterAudioEngineObserver(AudioEngineObserver& observer)
{
    VOIP_TRACE(kTraceInfo, kTraceVoice, AoEId(_instanceId, -1),
                 "TransmitMixer::RegisterAudioEngineObserver()");
    CriticalSectionScoped cs(&_callbackCritSect);
  ...
  *max_channels = 1;
  for (ChannelManager::Iterator it(_channelManagerPtr); it.IsValid();
    ...
    if (channel->Sending()) {
      ...
      *max_sample_rate = std::max(*max_sample_rate, 8000);
      *max_channels = std::max(*max_channels, 1);
    }
  }
}
...
void TransmitMixer::DemuxAndMix(const int voe_channels[],
  ...
  for (int i = 0; i < number_of_voe_channels; ++i) {
    aoe::ChannelOwner ch = _channelManagerPtr->GetChannel(voe_channels[i]);
...
void TransmitMixer::GenerateAudioFrame(const int16_t* audio,
  ...
  // See: https://code.google.com/p/VoIP/issues/detail?id=3146
...
{
    // We check in AoEAudioProcessingImpl that this is only called when
