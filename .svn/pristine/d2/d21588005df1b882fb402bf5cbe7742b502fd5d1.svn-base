...
class RtpReceiverImpl : public RtpReceiver {
 public:
  ...
  int32_t CSRCs(uint32_t array_of_csrc[kCsrcSize]) const;

  int32_t Energy(uint8_t array_of_energy[kCsrcSize]) const;
 ...
 private:
  ...
  void CheckSSRCChanged(const AudioPacketHeader& rtp_header);
  void CheckCSRC(const WebRtcRTPHeader& rtp_header);
  int32_t CheckPayloadChanged(const AudioPacketHeader& rtp_header,
  ...
  uint32_t ssrc_;
  uint8_t num_csrcs_;
  uint32_t current_remote_csrc_[kCsrcSize];
