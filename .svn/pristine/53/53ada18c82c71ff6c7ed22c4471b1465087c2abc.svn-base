...
#include "audio_engine/system_wrappers/interface/ref_count.h"
...
#include <string.h>
 ...
 #if defined(VOIP_WINDOWS_CORE_AUDIO_BUILD)
    #include "audio_device_core_win.h"
 #endif
#elif defined(VOIP_ANDROID)
    #include <stdlib.h>
...
int32_t AudioDeviceModuleImpl::CheckPlatform()
{
    ...
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "current platform is WIN32");
#elif defined(VOIP_ANDROID)
    platform = kPlatformAndroid;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "current platform is ANDROID");
#elif defined(VOIP_LINUX)
    platform = kPlatformLinux;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "current platform is LINUX");
#elif defined(VOIP_IOS)
    platform = kPlatformIOS;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "current platform is IOS");
#elif defined(VOIP_MAC)
    platform = kPlatformMac;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "current platform is MAC");
#endif

    if (platform == kPlatformNotSupported)
    {
        VOIP_TRACE(kTraceCritical, kTraceAudioDevice, _id, "current platform is not supported => this module will self destruct!");
        return -1;
    }
...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects()
{
...
#if defined(VOIP_DUMMY_AUDIO_BUILD)
    ptrAudioDevice = new AudioDeviceDummy(Id());
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Dummy Audio APIs will be utilized");
        ...
        )
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Windows Wave APIs will be utilized");
    }
#if defined(VOIP_WINDOWS_CORE_AUDIO_BUILD)
    ...
    {
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "attempting to use the Windows Core Audio APIs...");

        if (AudioDeviceWindowsCore::CoreAudioIsSupported())
        {
            // create *Windows Core Audio* implementation
            ptrAudioDevice = new AudioDeviceWindowsCore(Id());
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Windows Core Audio APIs will be utilized");
        }
        else
        {
            // create *Windows Wave Audio* implementation
            ...
            if (ptrAudioDevice != NULL)
            {
                ...
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "Windows Core Audio is *not* supported => Wave APIs will be utilized instead");
            }
        }
    }
#endif // defined(VOIP_WINDOWS_CORE_AUDIO_BUILD)
    if (ptrAudioDevice != NULL)
    {
...
#if defined(VOIP_ANDROID_OPENSLES)
    ...
    {
#if defined(LINUX_PULSE)
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "attempting to use the Linux PulseAudio APIs...");
        ...
        if (pulseDevice->Init() != -1)
        {
            ...
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Linux PulseAudio APIs will be utilized");
        }
        else
        {
            delete pulseDevice;
            ...
            if (ptrAudioDevice != NULL)
            {
                ...
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "Linux PulseAudio is *not* supported => ALSA APIs will be utilized instead");
            }
#endif
#if defined(LINUX_PULSE)
        }
    ...
    else if (audioLayer == kLinuxAlsaAudio)
    {
        ...
        // create *Linux ALSA Audio* implementation
        ptrAudioDevice = new AudioDeviceLinuxALSA(Id());
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Linux ALSA APIs will be utilized");
#endif
    }
    ...
    if (audioLayer == kPlatformDefaultAudio)
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "iPhone Audio APIs will be utilized");
    }
    ...
    if (audioLayer == kPlatformDefaultAudio)
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Mac OS X Audio APIs will be utilized");
    }
...
#endif  // if defined(VOIP_DUMMY_AUDIO_BUILD)

    if (ptrAudioDevice == NULL)
    {
        VOIP_TRACE(kTraceCritical, kTraceAudioDevice, _id, "unable to create the platform specific audio device implementation");
        return -1;
    }

    if (ptrAudioDeviceUtility == NULL)
    {
        VOIP_TRACE(kTraceCritical, kTraceAudioDevice, _id, "unable to create the platform specific audio device utility");
        return -1;
    }
...
int32_t AudioDeviceModuleImpl::Process()
{
    ...
    if (_ptrAudioDevice->PlayoutWarning())
    {
        ...
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "=> OnWarningIsReported(kPlayoutWarning)");
    ...
    if (_ptrAudioDevice->PlayoutError())
    {
        ...
        if (_ptrCbAudioDeviceObserver)
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "=> OnErrorIsReported(kPlayoutError)");
    ...
    if (_ptrAudioDevice->RecordingWarning())
    {
        ...
        if (_ptrCbAudioDeviceObserver)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "=> OnWarningIsReported(kRecordingWarning)");
    ...
    if (_ptrAudioDevice->RecordingError())
    {
        ...
        if (_ptrCbAudioDeviceObserver)
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "=> OnErrorIsReported(kRecordingError)");
...
int32_t AudioDeviceModuleImpl::ActiveAudioLayer(AudioLayer* audioLayer) const
{
    ...
    *audioLayer = activeAudio;
    ...
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: kWindowsWaveAudio");
    }
    else if (*audioLayer == AudioDeviceModule::kWindowsCoreAudio)
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: kWindowsCoreAudio");
    }
    else if (*audioLayer == AudioDeviceModule::kLinuxAlsaAudio)
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: kLinuxAlsaAudio");
    }
    else
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: NOT_SUPPORTED");
    }
...
bool AudioDeviceModuleImpl::Initialized() const
{
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: %d", _initialized);
    return (_initialized);
}
...
int32_t AudioDeviceModuleImpl::SpeakerVolumeIsAvailable(bool* available)
{
    ...
    *available = isAvailable;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: available=%d", *available);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::SpeakerVolume(uint32_t* volume) const
{
    ...
    *volume = level;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: volume=%u", *volume);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::WaveOutVolume(uint16_t* volumeLeft, uint16_t* volumeRight) const
{
    ...
    *volumeRight = volRight;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "outputs: volumeLeft=%u, volumeRight=%u",
...
bool AudioDeviceModuleImpl::SpeakerIsInitialized() const
{
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: %d", isInitialized);
...
int32_t AudioDeviceModuleImpl::MaxSpeakerVolume(uint32_t* maxVolume) const
{
    ...
    *maxVolume = maxVol;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: maxVolume=%d", *maxVolume);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::MinSpeakerVolume(uint32_t* minVolume) const
{
    ...
    *minVolume = minVol;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: minVolume=%u", *minVolume);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::SpeakerVolumeStepSize(uint16_t* stepSize) const
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the speaker-volume step size");
        return -1;
    }

    *stepSize = delta;

    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: stepSize=%u", *stepSize);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::SpeakerMute(bool* enabled) const
{
    ...
    *enabled = muted;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: enabled=%u", *enabled);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::MicrophoneVolume(uint32_t* volume) const
{
    ...
    *volume = level;
    ...
    VOIP_TRACE(kTraceStream, kTraceAudioDevice, _id, "output: volume=%u", *volume);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::SetStereoRecording(bool enable)
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "recording in stereo is not supported");
        return -1;
    }

    if (_ptrAudioDevice->SetStereoRecording(enable) == -1)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to enable stereo recording");
        return -1;
    }
...
int32_t AudioDeviceModuleImpl::SetStereoPlayout(bool enable)
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "unable to set stereo mode while playing side is initialized");
        return -1;
    }

    if (_ptrAudioDevice->SetStereoPlayout(enable))
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "stereo playout is not supported");
        return -1;
    }
...
int32_t AudioDeviceModuleImpl::MaxMicrophoneVolume(uint32_t* maxVolume) const
{
    ...
    *maxVolume = maxVol;
    ...
    VOIP_TRACE(kTraceStream, kTraceAudioDevice, _id, "output: maxVolume=%d", *maxVolume);
    return (0);
}
...
int16_t AudioDeviceModuleImpl::PlayoutDevices()
{
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: #playout devices=%d", nPlayoutDevices);
    ...
    char guid[kAdmMaxGuidSize])
{
    ...
    if (name != NULL)
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: name=%s", name);
    }
    if (guid != NULL)
    {
        VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: guid=%s", guid);
    }
...
int16_t AudioDeviceModuleImpl::RecordingDevices()
{
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id,
...
int32_t AudioDeviceModuleImpl::SetPlayoutBuffer(const BufferType type, uint16_t sizeMS)
{
    ...
    if (_ptrAudioDevice->PlayoutIsInitialized())
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "unable to modify the playout buffer while playing side is initialized");
        return -1;
    }
    ...
    if (kFixedBufferSize == type)
    {
        ...
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "size parameter is out of range");
            return -1;
        }
    }
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to set the playout buffer (error: %d)", LastError());
    }
...
int32_t AudioDeviceModuleImpl::PlayoutBuffer(BufferType* type, uint16_t* sizeMS) const
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the buffer type and size");
        return -1;
    }
    ...
    *sizeMS = size;
    ...
    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: type=%u, sizeMS=%u", *type, *sizeMS);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::PlayoutDelay(uint16_t* delayMS) const
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the playout delay");
        return -1;
    }

    *delayMS = delay;

    VOIP_TRACE(kTraceStream, kTraceAudioDevice, _id, "output: delayMS=%u", *delayMS);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::RecordingDelay(uint16_t* delayMS) const
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the recording delay");
        return -1;
    }
...
int32_t AudioDeviceModuleImpl::CPULoad(uint16_t* load) const
{
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the CPU load");
        return -1;
    }

    *load = cpuLoad;

    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: load=%u", *load);
    return (0);
}
...
int32_t AudioDeviceModuleImpl::RecordingSampleRate(uint32_t* samplesPerSec) const
{
    ...
    if (sampleRate == -1)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to retrieve the sample rate");
        return -1;
    }

    *samplesPerSec = sampleRate;

    VOIP_TRACE(kTraceStateInfo, kTraceAudioDevice, _id, "output: samplesPerSec=%u", *samplesPerSec);
    return (0);
}
