...
{
    seed[0] = (seed[0] * ((int32_t)69069) + 1) & (VOIP_SPL_MAX_SEED_USED - 1);
    return seed[0];
}

int16_t VoipSpl_RandU(uint32_t *seed)
{
    return (int16_t)(VoipSpl_IncreaseSeed(seed) >> 16);
}

int16_t VoipSpl_RandN(uint32_t *seed)
{
    return kRandNTable[VoipSpl_IncreaseSeed(seed) >> 23];
}
