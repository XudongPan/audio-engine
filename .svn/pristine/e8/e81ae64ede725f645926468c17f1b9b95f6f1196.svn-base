...
#include "audio_engine/modules/audio_package/source/audio_pkg_receiver_audio.h"
...
int32_t RTPReceiverAudio::ParseRtpPacket(WebRtcRTPHeader* rtp_header,
  ...
  if (rtp_header->type.Audio.numEnergy > 0 &&
      rtp_header->type.Audio.numEnergy <= kCsrcSize) {
...
int RTPReceiverAudio::Energy(uint8_t array_of_energy[kCsrcSize]) const {
  CriticalSectionScoped cs(crit_sect_.get());

  assert(num_energy_ <= kCsrcSize);
