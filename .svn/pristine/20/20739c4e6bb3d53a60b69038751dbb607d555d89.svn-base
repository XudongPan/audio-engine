...
void* GainControlImpl::CreateHandle() const {
  Handle* handle = NULL;
  if (VoipAgc_Create(&handle) != apm_->kNoError) {
    handle = NULL;
  } else {
    assert(handle != NULL);
  }
...
int GainControlImpl::DestroyHandle(void* handle) const {
  return VoipAgc_Free(static_cast<Handle*>(handle));
}

int GainControlImpl::InitializeHandle(void* handle) const {
  return VoipAgc_Init(static_cast<Handle*>(handle),
...
int GainControlImpl::ConfigureHandle(void* handle) const {
  ...
  return VoipAgc_set_config(static_cast<Handle*>(handle), config);
}
