...
// Validates the |candidate_delay|, estimated in VoIP_ProcessBinarySpectrum(),
...
// VoIP_ProcessBinarySpectrum().  The algorithm takes the
...
BinaryDelayEstimatorFarend* Voip_CreateBinaryDelayEstimatorFarend(
  ...
  if (self != NULL) {
    ...
    if (malloc_fail) {
      Voip_FreeBinaryDelayEstimatorFarend(self);
      self = NULL;
    }
  }
...
void Voip_AddBinaryFarSpectrum(BinaryDelayEstimatorFarend* handle,
...
BinaryDelayEstimator* Voip_CreateBinaryDelayEstimator(
  ...
  if (self != NULL) {
    ...
    if (malloc_fail) {
      Voip_FreeBinaryDelayEstimator(self);
      self = NULL;
    }
  }
...
int Voip_ProcessBinarySpectrum(BinaryDelayEstimator* self,
  ...
  for (i = 0; i < self->farend->history_size; i++) {
    ...
    if (self->farend->far_bit_counts[i] > 0) {
      ...
      Voip_MeanEstimatorFix(bit_count, shifts, &(self->mean_bit_counts[i]));
    }
  }
