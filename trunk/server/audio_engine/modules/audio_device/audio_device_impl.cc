...
#endif
#include "audio_engine/modules/audio_device/dummy/audio_device_dummy.h"
#include "audio_engine/modules/audio_device/dummy/audio_device_utility_dummy.h"
												 ...
												 AudioDeviceGeneric *device)
{
    ...
    if (audioDevice->CreatePlatformSpecificObjects(device) == -1)
    {
        delete audioDevice;
        return NULL;
    }
...
int32_t AudioDeviceModuleImpl::CreatePlatformSpecificObjects(AudioDeviceGeneric* device)
{
    WEBRTC_TRACE(kTraceInfo, kTraceAudioDevice, _id, "%s", __FUNCTION__);
