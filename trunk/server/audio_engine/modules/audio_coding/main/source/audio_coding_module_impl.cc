...
#include "audio_engine/modules/audio_coding/main/source/audio_coding_module_impl.h"
...
#include "audio_engine/modules/audio_coding/main/acm2/call_statistics.h"
#include "audio_engine/modules/audio_coding/main/source/acm_dtmf_detection.h"
#include "audio_engine/modules/audio_coding/main/source/acm_generic_codec.h"
#include "audio_engine/modules/audio_coding/main/source/acm_resampler.h"
#include "audio_engine/modules/audio_coding/main/acm2/nack.h"
#include "audio_engine/system_wrappers/interface/clock.h"
...
AudioCodingModuleImpl::AudioCodingModuleImpl(const int32_t id, Clock* clock)
  ...
  strncpy(send_codec_inst_.plname, no_name, PAYLOAD_NAME_SIZE - 1);
  ...
  if (InitializeReceiverSafe() < 0) {
    WEBRTC_TRACE(VoIP::kTraceError, VoIP::kTraceAudioCoding, id_,
                 "Cannot initialize receiver");
  }
  WEBRTC_TRACE(VoIP::kTraceMemory, VoIP::kTraceAudioCoding, id, "Created");
}
...
AudioCodingModuleImpl::~AudioCodingModuleImpl() {
  ...
  acm_crit_sect_ = NULL;
  WEBRTC_TRACE(VoIP::kTraceMemory, VoIP::kTraceAudioCoding, id_,
               "Destroyed");
}
...
static int IsValidSendCodec(const CodecInst& send_codec,
  ...
  if ((send_codec.channels != 1) && (send_codec.channels != 2)) {
    WEBRTC_TRACE(VoIP::kTraceError, VoIP::kTraceAudioCoding, acm_id,
...
int32_t AudioCodingModuleImpl::RegisterSendCodec(
  ...
  if (send_codec.channels == 2) {
    ...
    if (vad_enabled_ || dtx_enabled_) {
      WEBRTC_TRACE(VoIP::kTraceWarning, VoIP::kTraceAudioCoding, id_,
  ...
  if (!is_send_codec) {
    ...
    if (send_codec_inst_.packsize != send_codec.packsize) {
      force_init = true;
    }
    ...
    if (force_init) {
      ...
      send_codec_inst_.packsize = send_codec.packsize;
...
int32_t AudioCodingModuleImpl::SendCodec(
  ...
  WEBRTC_TRACE(VoIP::kTraceStream, VoIP::kTraceAudioCoding, id_,
               "SendCodec()");
  CriticalSectionScoped lock(acm_crit_sect_);
...
int32_t AudioCodingModuleImpl::Add10MsData(
  ...
  TRACE_EVENT_ASYNC_BEGIN1("VoIP", "Audio", ptr_frame->timestamp_,
...
int32_t AudioCodingModuleImpl::RegisterReceiveCodec(
  ...
  // http://code.google.com/p/VoIP/issues/detail?id=453
...
int32_t AudioCodingModuleImpl::RegisterRecCodecMSSafe(
  ...
  if (!codecs[codec_id]->DecoderInitialized()) {
    ...
    if (status < 0) {
      ...
      WEBRTC_TRACE(
          VoIP::kTraceError, VoIP::kTraceAudioCoding, id_,
...
int32_t AudioCodingModuleImpl::PlayoutData10Ms(
  ...
  TRACE_EVENT_ASYNC_BEGIN0("VoIP", "ACM::PlayoutData10Ms", this);
  bool stereo_mode;

  if (GetSilence(desired_freq_hz, audio_frame)) {
     TRACE_EVENT_ASYNC_END1("VoIP", "ACM::PlayoutData10Ms", this,
                            "silence", true);
  ...
  // Limit the scope of ACM Critical section.
  {
    ...
    if ((receive_freq != desired_freq_hz) && (desired_freq_hz != -1)) {
      TRACE_EVENT_ASYNC_END2("VoIP", "ACM::PlayoutData10Ms", this,
...
void AudioCodingModuleImpl::DestructEncoderInst(void* inst) {
  WEBRTC_TRACE(VoIP::kTraceDebug, VoIP::kTraceAudioCoding, id_,
               "DestructEncoderInst()");
...
int32_t AudioCodingModuleImpl::IncomingPayload(
  ...
  if (dummy_rtp_header_ == NULL) {
    ...
    recv_pl_frame_size_smpls_ = codec_params.codec_inst.packsize;
  }
...
int16_t AudioCodingModuleImpl::DecoderParamByPlType(
  ...
  codec_params.codec_inst.packsize = 0;
