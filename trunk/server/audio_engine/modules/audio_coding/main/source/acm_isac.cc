...
#include "audio_engine/modules/audio_coding/main/source/acm_neteq.h"
#include "audio_engine/modules/audio_coding/neteq/interface/webrtc_neteq.h"
#include "audio_engine/modules/audio_coding/neteq/interface/webrtc_neteq_help_macros.h"
  ...
  *bitstream_len_byte = 0;
  while ((*bitstream_len_byte == 0) && (in_audio_ix_read_ < frame_len_smpl_)) {
    if (in_audio_ix_read_ > in_audio_ix_write_) {
      // something is wrong.
      WEBRTC_TRACE(VoIP::kTraceError, VoIP::kTraceAudioCoding, unique_id_,
  ...
  if (*bitstream_len_byte == 0) {
    WEBRTC_TRACE(VoIP::kTraceWarning, VoIP::kTraceAudioCoding, unique_id_,
...
int16_t ACMISAC::InternalInitEncoder(WebRtcACMCodecParams* codec_params) {
  ...
  if (isac_coding_mode_ == CHANNEL_INDEPENDENT) {
    if (ACM_ISAC_CONTROL(codec_inst_ptr_->inst,
                         ...
                         codec_params->codec_inst.packsize /
...
int16_t ACMISAC::InternalInitDecoder(WebRtcACMCodecParams* codec_params) {
  ...
  if (!encoder_initialized_) {
    ...
    codec_params->codec_inst.packsize = kIsacPacSize960;
...
int16_t ACMISAC::UpdateEncoderSampFreq(
  ...
  if (current_samp_rate_hz != encoder_samp_freq_hz) {
    if ((encoder_samp_freq_hz != 16000) &&
      ...
      frame_len_smpl_ = ACM_ISAC_GETNEWFRAMELEN(codec_inst_ptr_->inst);
      encoder_params_.codec_inst.packsize = frame_len_smpl_;
