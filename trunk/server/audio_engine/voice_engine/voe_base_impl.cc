...
AoEBase* AoEBase::GetInterface(AudioEngine* voiceEngine)
{
    if (NULL == voiceEngine)
    {
        return NULL;
...
int VoEBaseImpl::RegisterAudioEngineObserver(AudioEngineObserver& observer)
{
...
int VoEBaseImpl::DeRegisterAudioEngineObserver()
{
    WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
					  ...
					  AudioDeviceGeneric * generic)
{
    ...
    if (external_adm == NULL)
    {
		...
		AudioDeviceModule *device = NULL;
		device = AudioDeviceModuleImpl::Create(
			VoEId(_shared->instance_id(), -1), _shared->audio_device_layer(), generic);

        _shared->set_audio_device(device);
