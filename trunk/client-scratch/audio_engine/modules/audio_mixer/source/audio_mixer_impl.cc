...
#include "audio_engine/modules/audio_mixer/source/audio_mixer_impl.h"
#include "audio_engine/modules/audio_mixer/source/audio_frame_manipulator.h"
...
int32_t AudioConferenceMixerImpl::TimeUntilNextProcess() {
    ...
    if(_timeScheduler.TimeToNextUpdate(timeUntilNextProcess) != 0) {
        VOIP_TRACE(kTraceError, kTraceAudioMixerServer, _id,
...
int32_t AudioConferenceMixerImpl::Process() {
    ...
    if(_audioFramePool->PopMemory(mixedAudio) == -1) {
        VOIP_TRACE(kTraceMemory, kTraceAudioMixerServer, _id,
...
int32_t AudioConferenceMixerImpl::RegisterMixerStatusCallback(
    ...
    }
    {
        CriticalSectionScoped cs(_cbCrit.get());
        if(_mixerStatusCallback != NULL) {
            VOIP_TRACE(kTraceWarning, kTraceAudioMixerServer, _id,
...
void AudioConferenceMixerImpl::UpdateToMix(
    ...
    VOIP_TRACE(kTraceStream, kTraceAudioMixerServer, _id,
