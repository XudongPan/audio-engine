                                        ...
                                        uint32_t formatTag)
{
    ...
    case kWaveFormatALaw:
        ...
        codec_info_.packsize  = codec_info_.plfreq / 100;
        break;
    case kWaveFormatMuLaw:
        ...
        codec_info_.packsize  = codec_info_.plfreq / 100;
         break;
    case kWaveFormatPcm:
        codec_info_.packsize  = (bitsPerSample * (codec_info_.plfreq / 100)) / 8;
    ...
    const uint32_t bufferSize)
{
 ...
 bufferSize= %d)",
    ...
    if(outData == NULL)
    {
        VOIP_TRACE(kTraceError, kTraceFile, _id, "output buffer NULL");
    }
    ...
    const CodecInst& codecInst)
{
    ...
    if(set_codec_info(codecInst) != 0)
    {
        VOIP_TRACE(kTraceError, kTraceFile, _id, "CodecInst not recognized!");
        return -1;
    }
    _writing = true;
    _bytesWritten = 1;
    ...
    const uint32_t dataLength)
{
 ...
 dataLen= %d)",
    ...
    if(buffer == NULL)
    {
        VOIP_TRACE(kTraceError, kTraceFile, _id,"buffer NULL");
    }
...
#if defined(VOIP_CODEC_AMR) || defined(VOIP_CODEC_AMRWB) || \
    defined(VOIP_CODEC_ILBC)
    int16_t read_len = 0;
#endif
    _codecId = kCodecNoCodec;
    _playoutPositionMs = 0;
                                              ...
                                              uint32_t bufferSize)
{
 ...
 bytes=%ld)",
    ...
    if(! _reading)
    {
        VOIP_TRACE(kTraceError, kTraceFile, _id, "not currently reading!");
        return -1;
    }
...
int32_t ModuleFileUtility::InitCompressedWriting(
    ...
    const CodecInst& codecInst)
{
    VOIP_TRACE(kTraceDebug, kTraceFile, _id,
 ...
 codecName= %s)",
    ...
    if(STR_CASE_CMP(codecInst.plname, "ilbc") == 0)
    {
        ...
        else if(codecInst.packsize == 240)
        {
            _codecId = kCodecIlbc30Ms;
            out.Write("#!iLBC30\n",9);
        }
...
#endif
#if(defined(VOIP_CODEC_ISAC) || defined(VOIP_CODEC_ISACFX))
                                          ...
                                          const uint32_t freqInHz)
{
    ...
    if(fileName == NULL)
    {
        VOIP_TRACE(kTraceError, kTraceFile, _id, "filename NULL");
        return -1;
    }
