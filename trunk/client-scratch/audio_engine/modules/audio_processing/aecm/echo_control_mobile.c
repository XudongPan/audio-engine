...
static int VoipAecm_EstBufDelay(aecmob_t *aecmInst, short msInSndCardBuf);

// Stuffs the farend buffer if the estimated delay is too large
static int VoipAecm_DelayComp(aecmob_t *aecmInst);

int32_t VoipAecm_Create(void **aecmInst)
{
    aecmob_t *aecm;
    if (aecmInst == NULL)
    {
    ...
    *aecmInst = aecm;
    ...
    if (VoipAecm_CreateCore(&aecm->aecmCore) == -1)
    {
        VoipAecm_Free(aecm);
        aecm = NULL;
        return -1;
    ...
    aecm->farendBuf = Voip_CreateBuffer(kBufSizeSamp,
                                          sizeof(int16_t));
    if (!aecm->farendBuf)
    {
        VoipAecm_Free(aecm);
...
int32_t VoipAecm_Free(void *aecmInst)
{
...
#endif // AEC_DEBUG
    VoipAecm_FreeCore(aecm->aecmCore);
    Voip_FreeBuffer(aecm->farendBuf);
    free(aecm);

    return 0;
}

int32_t VoipAecm_Init(void *aecmInst, int32_t sampFreq)
{
    aecmob_t *aecm = aecmInst;
    ...
    if (VoipAecm_InitCore(aecm->aecmCore, aecm->sampFreq) == -1)
    {
    ...
    if (Voip_InitBuffer(aecm->farendBuf) == -1)
    {
        aecm->lastError = AECM_UNSPECIFIED_ERROR;
        return -1;
    }
    ...
    if (VoipAecm_set_config(aecm, aecConfig) == -1)
    {
        aecm->lastError = AECM_UNSPECIFIED_ERROR;
        return -1;
    }
...
int32_t VoipAecm_BufferFarend(void *aecmInst, const int16_t *farend,
                                int16_t nrOfSamples)
{
    aecmob_t *aecm = aecmInst;
    int32_t retVal = 0;
    ...
    if (!aecm->ECstartup)
    {
        VoipAecm_DelayComp(aecm);
    }

    Voip_WriteBuffer(aecm->farendBuf, farend, (size_t) nrOfSamples);

    return retVal;
}

int32_t VoipAecm_Process(void *aecmInst, const int16_t *nearendNoisy,
                           ...
                           int16_t nrOfSamples, int16_t msInSndCardBuf)
{
    ...
    if (aecm->ECstartup)
    {
        ...
        nmbrOfFilledBuffers =
            (short) Voip_available_read(aecm->farendBuf) / FRAME_LEN;
        ...
        if (aecm->checkBuffSize)
        {
            ...
            if (abs(aecm->firstVal - aecm->msInSndCardBuf)
                    < VOIP_SPL_MAX(0.2 * aecm->msInSndCardBuf, kSampMsNb))
            {
            ...
            if (aecm->checkBufSizeCtr * nBlocks10ms > 50)
            {
                ...
                aecm->bufSizeStart = VOIP_SPL_MIN((3 * aecm->msInSndCardBuf
        ...
        if (!aecm->checkBuffSize)
        {
            ...
            {
                Voip_MoveReadPtr(aecm->farendBuf,
                                   (int) Voip_available_read(aecm->farendBuf)
    ...
    } else
    {
        ...
        for (i = 0; i < nFrames; i++)
        {
            ...
            if (nmbrOfFilledBuffers > 0)
            {
                ...
                Voip_ReadBuffer(aecm->farendBuf, (void**) &farend_ptr, farend,
                                  FRAME_LEN);
            ...
            {
                VoipAecm_EstBufDelay(aecm, aecm->msInSndCardBuf);
            }

            // Call the AECM
            /*VoipAecm_ProcessFrame(aecm->aecmCore, farend, &nearend[FRAME_LEN * i],
             &out[FRAME_LEN * i], aecm->knownDelay);*/
            if (VoipAecm_ProcessFrame(aecm->aecmCore,
                                        farend_ptr,
...
#ifdef AEC_DEBUG
    msInAECBuf = (short) Voip_available_read(aecm->farendBuf) /
...
int32_t VoipAecm_set_config(void *aecmInst, AecmConfig config)
{
    aecmob_t *aecm = aecmInst;
...
int32_t VoipAecm_get_config(void *aecmInst, AecmConfig *config)
{
    aecmob_t *aecm = aecmInst;
...
int32_t VoipAecm_InitEchoPath(void* aecmInst,
                                ...
                                size_t size_bytes)
{
    ...
    if (size_bytes != VoipAecm_echo_path_size_bytes())
    {
    ...
    VoipAecm_InitEchoPathCore(aecm->aecmCore, echo_path_ptr);

    return 0;
}

int32_t VoipAecm_GetEchoPath(void* aecmInst,
                               void* echo_path,
                               size_t size_bytes)
{
    aecmob_t *aecm = aecmInst;
...
size_t VoipAecm_echo_path_size_bytes()
{
    return (PART_LEN1 * sizeof(int16_t));
}

int32_t VoipAecm_get_error_code(void *aecmInst)
{
    aecmob_t *aecm = aecmInst;
...
static int VoipAecm_EstBufDelay(aecmob_t *aecm, short msInSndCardBuf)
{
    short delayNew, nSampSndCard;
    short nSampFar = (short) Voip_available_read(aecm->farendBuf);
    short diff;
    ...
    {
        Voip_MoveReadPtr(aecm->farendBuf, FRAME_LEN);
        delayNew += FRAME_LEN;
    }

    aecm->filtDelay = VOIP_SPL_MAX(0, (8 * aecm->filtDelay + 2 * delayNew) / 10);
    ...
    {
        aecm->knownDelay = VOIP_SPL_MAX((int)aecm->filtDelay - 160, 0);
    }
    return 0;
}

static int VoipAecm_DelayComp(aecmob_t *aecm)
{
    int nSampFar = (int) Voip_available_read(aecm->farendBuf);
    ...
    if (delayNew > FAR_BUF_LEN - FRAME_LEN * aecm->aecmCore->mult)
    {
        ...
        nSampAdd = (int)(VOIP_SPL_MAX(((nSampSndCard >> 1) - nSampFar),
                FRAME_LEN));
        nSampAdd = VOIP_SPL_MIN(nSampAdd, maxStuffSamp);

        Voip_MoveReadPtr(aecm->farendBuf, -nSampAdd);
