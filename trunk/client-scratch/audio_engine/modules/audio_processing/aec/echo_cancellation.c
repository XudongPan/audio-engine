 ...
 */
#include "audio_engine/modules/audio_processing/aec/include/echo_cancellation.h"
...
#include "audio_engine/modules/audio_processing/aec/aec_resampler.h"
#include "audio_engine/modules/audio_processing/aec/echo_cancellation_internal.h"
...
// fixed delay.
//
// VOIP_UNTRUSTED_DELAY will only have an impact when |extended_filter_mode|
...
#if defined(VOIP_BUILD) && defined(VOIP_MAC)
#define VOID_UNTRUSTED_DELAY
...
#endif
#if !defined(VOID_UNTRUSTED_DELAY)
...
int32_t VoipAec_Create(void** aecInst) {
  aecpc_t* aecpc;
  if (aecInst == NULL) {
    return -1;
  }
  ...
  *aecInst = aecpc;
  ...
  if (VoipAec_CreateAec(&aecpc->aec) == -1) {
    VoipAec_Free(aecpc);
    aecpc = NULL;
    return -1;
  }

  if (VoipAec_CreateResampler(&aecpc->resampler) == -1) {
    VoipAec_Free(aecpc);
    aecpc = NULL;
    return -1;
  }
  ...
  aecpc->far_pre_buf =
      Voip_CreateBuffer(PART_LEN2 + kResamplerBufferSize, sizeof(float));
  ...
  aecpc->far_pre_buf_s16 =
      Voip_CreateBuffer(PART_LEN2 + kResamplerBufferSize, sizeof(int16_t));
  ...
  {
    char filename[64];
    sprintf(filename, "aec_buf%d.dat", voip_aec_instance_count);
    aecpc->bufFile = fopen(filename, "wb");
    sprintf(filename, "aec_skew%d.dat", voip_aec_instance_count);
    aecpc->skewFile = fopen(filename, "wb");
    sprintf(filename, "aec_delay%d.dat", voip_aec_instance_count);
    aecpc->delayFile = fopen(filename, "wb");
    voip_aec_instance_count++;
  }
#endif
...
int32_t VoipAec_Free(void* aecInst) {
  ...
  Voip_FreeBuffer(aecpc->far_pre_buf);

#ifdef VOIP_AEC_DEBUG_DUMP
  Voip_FreeBuffer(aecpc->far_pre_buf_s16);
  fclose(aecpc->bufFile);
  fclose(aecpc->skewFile);
  fclose(aecpc->delayFile);
#endif

  VoipAec_FreeAec(aecpc->aec);
  VoipAec_FreeResampler(aecpc->resampler);
  free(aecpc);

  return 0;
}

int32_t VoipAec_Init(void* aecInst, int32_t sampFreq, int32_t scSampFreq) {
  aecpc_t* aecpc = aecInst;
  AecConfig aecConfig;
  ...
  if (VoipAec_InitAec(aecpc->aec, aecpc->sampFreq) == -1) {
    aecpc->lastError = AEC_UNSPECIFIED_ERROR;
    return -1;
  }

  if (VoipAec_InitResampler(aecpc->resampler, aecpc->scSampFreq) == -1) {
    aecpc->lastError = AEC_UNSPECIFIED_ERROR;
    return -1;
  }

  if (Voip_InitBuffer(aecpc->far_pre_buf) == -1) {
    aecpc->lastError = AEC_UNSPECIFIED_ERROR;
    return -1;
  }
  Voip_MoveReadPtr(aecpc->far_pre_buf, -PART_LEN);  // Start overlap.
  ...
  if (VoipAec_set_config(aecpc, aecConfig) == -1) {
    aecpc->lastError = AEC_UNSPECIFIED_ERROR;
    return -1;
  }

#ifdef VOIP_AEC_DEBUG_DUMP
  if (Voip_InitBuffer(aecpc->far_pre_buf_s16) == -1) {
    aecpc->lastError = AEC_UNSPECIFIED_ERROR;
    return -1;
  }
  Voip_MoveReadPtr(aecpc->far_pre_buf_s16, -PART_LEN);  // Start overlap.
#endif
...
int32_t VoipAec_BufferFarend(void* aecInst,
  ...
  if (aecpc->skewMode == kAecTrue && aecpc->resample == kAecTrue) {
    ...
    VoipAec_ResampleLinear(aecpc->resampler,
                             farend,
                             nrOfSamples,
                             skew,
                             newFarend,
  ...
  VoipAec_SetSystemDelay(aecpc->aec,
                           VoipAec_system_delay(aecpc->aec) + newNrOfSamples);
  ...
  Voip_WriteBuffer(aecpc->far_pre_buf, farend_float, (size_t)newNrOfSamples);

  // Transform to frequency domain if we have enough data.
  while (Voip_available_read(aecpc->far_pre_buf) >= PART_LEN2) {
    // We have enough data to pass to the FFT, hence read PART_LEN2 samples.
    Voip_ReadBuffer(
        aecpc->far_pre_buf, (void**)&farend_float, tmp_farend, PART_LEN2);

    VoipAec_BufferFarendPartition(aecpc->aec, farend_float);

    // Rewind |far_pre_buf| PART_LEN samples for overlap before continuing.
    Voip_MoveReadPtr(aecpc->far_pre_buf, -PART_LEN);
#ifdef VOIP_AEC_DEBUG_DUMP
    Voip_ReadBuffer(
        aecpc->far_pre_buf_s16, (void**)&farend_ptr, newFarend, PART_LEN2);
    Voip_WriteBuffer(
        VoipAec_far_time_buf(aecpc->aec), &farend_ptr[PART_LEN], 1);
    Voip_MoveReadPtr(aecpc->far_pre_buf_s16, -PART_LEN);
#endif
  }
...
int32_t VoipAec_Process(void* aecInst,
  ...
  if (VoipAec_delay_correction_enabled(aecpc->aec)) {
    ProcessExtended(
...
#ifdef VOIP_AEC_DEBUG_DUMP
  {
    int16_t far_buf_size_ms = (int16_t)(VoipAec_system_delay(aecpc->aec) /
...
int VoipAec_set_config(void* handle, AecConfig config) {
  ...
  VoipAec_SetConfigCore(
      self->aec, config.nlpMode, config.metricsMode, config.delay_logging);
  return 0;
}

int VoipAec_get_echo_status(void* handle, int* status) {
  aecpc_t* self = (aecpc_t*)handle;
  if (status == NULL) {
  ...
  *status = VoipAec_echo_state(self->aec);

  return 0;
}

int VoipAec_GetMetrics(void* handle, AecMetrics* metrics) {
  ...
  VoipAec_GetEchoStats(self->aec, &erl, &erle, &a_nlp);
...
int VoipAec_GetDelayMetrics(void* handle, int* median, int* std) {
  aecpc_t* self = handle;
  if (median == NULL) {
    self->lastError = AEC_NULL_POINTER_ERROR;
    return -1;
  ...
  if (VoipAec_GetDelayMetricsCore(self->aec, median, std) == -1) {
    // Logging disabled.
...
int32_t VoipAec_get_error_code(void* aecInst) {
  aecpc_t* aecpc = aecInst;
  return aecpc->lastError;
}

AecCore* VoipAec_aec_core(void* handle) {
  if (!handle) {
    return NULL;
  }
...
static int ProcessNormal(aecpc_t* aecpc,
  ...
  if (aecpc->skewMode == kAecTrue) {
    if (aecpc->skewFrCtr < 25) {
      ...
      retVal = VoipAec_GetSkew(aecpc->resampler, skew, &aecpc->skew);
      if (retVal == -1) {
        aecpc->skew = 0;
        aecpc->lastError = AEC_BAD_PARAMETER_WARNING;
      }
  ...
  if (aecpc->startup_phase) {
    ...
    if (aecpc->checkBuffSize) {
      ...
      if (abs(aecpc->firstVal - aecpc->msInSndCardBuf) <
          VOIP_SPL_MAX(0.2 * aecpc->msInSndCardBuf, sampMsNb)) {
      ...
      if (aecpc->counter * nBlocks10ms >= 6) {
        ...
        aecpc->bufSizeStart =
            VOIP_SPL_MIN((3 * aecpc->sum * aecpc->rate_factor * 8) /
      ...
      if (aecpc->checkBufSizeCtr * nBlocks10ms > 50) {
        ...
        // more than 0.5 sec.
        aecpc->bufSizeStart = VOIP_SPL_MIN(
    ...
    if (!aecpc->checkBuffSize) {
      ...
      int overhead_elements =
          VoipAec_system_delay(aecpc->aec) / PART_LEN - aecpc->bufSizeStart;
      if (overhead_elements == 0) {
        ...
        // has been done.
        VoipAec_MoveFarReadPtr(aecpc->aec, overhead_elements);
    ...
    for (i = 0; i < nFrames; i++) {
      // Call the AEC.
      VoipAec_ProcessFrame(aecpc->aec,
                             &nearend[FRAME_LEN * i],
...
static void ProcessExtended(aecpc_t* self,
  ...
  if (self->startup_phase) {
    ...
    int overhead_elements = (VoipAec_system_delay(self->aec) -
                             startup_size_ms / 2 * self->rate_factor * 8) /
                            PART_LEN;
    VoipAec_MoveFarReadPtr(self->aec, overhead_elements);
...

  {
    ...
    const int adjusted_known_delay =
        VOIP_SPL_MAX(0, self->knownDelay + delay_diff_offset);

    for (i = 0; i < num_frames; ++i) {
      VoipAec_ProcessFrame(self->aec,
                             &near[FRAME_LEN * i],
...
static void EstBufDelayNormal(aecpc_t* aecpc) {
  ...
  int current_delay = nSampSndCard - VoipAec_system_delay(aecpc->aec);
  ...
  if (current_delay < PART_LEN) {
    current_delay += VoipAec_MoveFarReadPtr(aecpc->aec, 1) * PART_LEN;
  }
  ...
  aecpc->filtDelay =
      VOIP_SPL_MAX(0, (short)(0.8 * aecpc->filtDelay + 0.2 * current_delay));
  ...
  if (aecpc->timeForDelayChange > 25) {
    aecpc->knownDelay = VOIP_SPL_MAX((int)aecpc->filtDelay - 160, 0);
  }
}

static void EstBufDelayExtended(aecpc_t* self) {
  int reported_delay = self->msInSndCardBuf * sampMsNb * self->rate_factor;
  int current_delay = reported_delay - VoipAec_system_delay(self->aec);
  int delay_difference = 0;
  ...
  if (current_delay < PART_LEN) {
    current_delay += VoipAec_MoveFarReadPtr(self->aec, 2) * PART_LEN;
  }

  if (self->filtDelay == -1) {
    self->filtDelay = VOIP_SPL_MAX(0, 0.5 * current_delay);
  } else {
    self->filtDelay = VOIP_SPL_MAX(
  ...
  if (self->timeForDelayChange > 25) {
    self->knownDelay = VOIP_SPL_MAX((int)self->filtDelay - 256, 0);
  }
}
