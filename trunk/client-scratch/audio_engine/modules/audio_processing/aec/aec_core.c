...
#include "audio_engine/modules/audio_processing/aec/aec_core_internal.h"
#include "audio_engine/modules/audio_processing/aec/aec_rdft.h"
...
const float VoipAec_weightCurve[65] = {
...
// fprintf(1, '\t%.4f, %.4f, %.4f, %.4f, %.4f, %.4f,\n', overDriveCurve);
const float VoipAec_overDriveCurve[65] = {
...
extern int voip_aec_instance_count;
#endif
...
int VoipAec_CreateAec(AecCore** aecInst) {
  ...
  *aecInst = aec;
  ...
  aec->nearFrBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN, sizeof(int16_t));
  if (!aec->nearFrBuf) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  ...
  aec->outFrBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN, sizeof(int16_t));
  if (!aec->outFrBuf) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  ...
  aec->nearFrBufH = Voip_CreateBuffer(FRAME_LEN + PART_LEN, sizeof(int16_t));
  if (!aec->nearFrBufH) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  ...
  aec->outFrBufH = Voip_CreateBuffer(FRAME_LEN + PART_LEN, sizeof(int16_t));
  if (!aec->outFrBufH) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  ...
  aec->far_buf =
      Voip_CreateBuffer(kBufSizePartitions, sizeof(float) * 2 * PART_LEN1);
  if (!aec->far_buf) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
...
#ifdef VOIP_AEC_DEBUG_DUMP
  aec->far_time_buf =
      Voip_CreateBuffer(kBufSizePartitions, sizeof(int16_t) * PART_LEN);
  if (!aec->far_time_buf) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  ...
  {
    char filename[64];
    sprintf(filename, "aec_far%d.pcm", voip_aec_instance_count);
    aec->farFile = fopen(filename, "wb");
    sprintf(filename, "aec_near%d.pcm", voip_aec_instance_count);
    aec->nearFile = fopen(filename, "wb");
    sprintf(filename, "aec_out%d.pcm", voip_aec_instance_count);
    aec->outFile = fopen(filename, "wb");
    sprintf(filename, "aec_out_linear%d.pcm", voip_aec_instance_count);
    aec->outLinearFile = fopen(filename, "wb");
  }
#endif
  aec->delay_estimator_farend =
      Voip_CreateDelayEstimatorFarend(PART_LEN1, kHistorySizeBlocks);
  if (aec->delay_estimator_farend == NULL) {
    VoipAec_FreeAec(aec);
    aec = NULL;
    return -1;
  }
  aec->delay_estimator = Voip_CreateDelayEstimator(
...
int VoipAec_FreeAec(AecCore* aec) {
  ...
  Voip_FreeBuffer(aec->nearFrBuf);
  Voip_FreeBuffer(aec->outFrBuf);

  Voip_FreeBuffer(aec->nearFrBufH);
  Voip_FreeBuffer(aec->outFrBufH);

  Voip_FreeBuffer(aec->far_buf);
  Voip_FreeBuffer(aec->far_buf_windowed);
#ifdef VOIP_AEC_DEBUG_DUMP
  Voip_FreeBuffer(aec->far_time_buf);
  fclose(aec->farFile);
  fclose(aec->nearFile);
  fclose(aec->outFile);
  fclose(aec->outLinearFile);
#endif
  Voip_FreeDelayEstimator(aec->delay_estimator);
  Voip_FreeDelayEstimatorFarend(aec->delay_estimator_farend);
...
static void OverdriveAndSuppress(AecCore* aec,
  ...
  for (i = 0; i < PART_LEN1; i++) {
    // Weight subbands
    if (hNl[i] > hNlFb) {
      hNl[i] = VoipAec_weightCurve[i] * hNlFb +
               (1 - VoipAec_weightCurve[i]) * hNl[i];
    }
    hNl[i] = powf(hNl[i], aec->overDriveSm * VoipAec_overDriveCurve[i]);
...
VoipAec_FilterFar_t VoipRtcAec_FilterFar;
VoipAec_ScaleErrorSignal_t VoipAec_ScaleErrorSignal;
VoipAec_FilterAdaptation_t VoipAec_FilterAdaptation;
VoipAec_OverdriveAndSuppress_t VoipAec_OverdriveAndSuppress;
VoipAec_ComfortNoise_t VoipAec_ComfortNoise;
...
int VoipAec_InitAec(AecCore* aec, int sampFreq) {
  ...
  if (Voip_InitBuffer(aec->nearFrBuf) == -1) {
    return -1;
  }

  if (Voip_InitBuffer(aec->outFrBuf) == -1) {
    return -1;
  }

  if (Voip_InitBuffer(aec->nearFrBufH) == -1) {
    return -1;
  }

  if (Voip_InitBuffer(aec->outFrBufH) == -1) {
    return -1;
  }

  // Initialize far-end buffers.
  if (Voip_InitBuffer(aec->far_buf) == -1) {
    return -1;
  }
  if (Voip_InitBuffer(aec->far_buf_windowed) == -1) {
    return -1;
  }
#ifdef VOIP_AEC_DEBUG_DUMP
  if (Voip_InitBuffer(aec->far_time_buf) == -1) {
    return -1;
  }
#endif
  aec->system_delay = 0;

  if (Voip_InitDelayEstimatorFarend(aec->delay_estimator_farend) != 0) {
    return -1;
  }
  if (Voip_InitDelayEstimator(aec->delay_estimator) != 0) {
    return -1;
  }
  ...
  Voip_set_allowed_offset(aec->delay_estimator, aec->num_partitions / 2);
  ...
  // this line then.
  Voip_enable_robust_validation(aec->delay_estimator, 1);
  ...
  VoipRtcAec_FilterFar = FilterFar;
  VoipAec_ScaleErrorSignal = ScaleErrorSignal;
  VoipAec_FilterAdaptation = FilterAdaptation;
  VoipAec_OverdriveAndSuppress = OverdriveAndSuppress;
  VoipAec_ComfortNoise = ComfortNoise;

#if defined(VOIP_ARCH_X86_FAMILY)
  if (Voip_GetCPUInfo(kSSE2)) {
    VoipAec_InitAec_SSE2();
  }
#endif

#if defined(MIPS_FPU_LE)
  VoipAec_InitAec_mips();
#endif
...
void VoipAec_BufferFarendPartition(AecCore* aec, const float* farend) {
  ...
  if (Voip_available_write(aec->far_buf) < 1) {
    VoipAec_MoveFarReadPtr(aec, 1);
  }
  ...
  Voip_WriteBuffer(aec->far_buf, &xf[0][0], 1);

  // Convert far-end partition to the frequency domain with windowing.
  memcpy(fft, farend, sizeof(float) * PART_LEN2);
  TimeToFrequency(fft, xf, 1);
  Voip_WriteBuffer(aec->far_buf_windowed, &xf[0][0], 1);
}

int VoipAec_MoveFarReadPtr(AecCore* aec, int elements) {
  int elements_moved = Voip_MoveReadPtr(aec->far_buf_windowed, elements);
  Voip_MoveReadPtr(aec->far_buf, elements);
#ifdef VOIP_AEC_DEBUG_DUMP
  Voip_MoveReadPtr(aec->far_time_buf, elements);
#endif
...
void VoipAec_ProcessFrame(AecCore* aec,
  ...
  Voip_WriteBuffer(aec->nearFrBuf, nearend, FRAME_LEN);
  // For H band
  if (aec->sampFreq == 32000) {
    Voip_WriteBuffer(aec->nearFrBufH, nearendH, FRAME_LEN);
  }
  ...
  if (aec->system_delay < FRAME_LEN) {
    ...
    VoipAec_MoveFarReadPtr(aec, -(aec->mult + 1));
  }

  // 2) Compensate for a possible change in the system delay.
  Voip_MoveReadPtr(aec->far_buf_windowed, move_elements);
  moved_elements = Voip_MoveReadPtr(aec->far_buf, move_elements);
  aec->knownDelay -= moved_elements * PART_LEN;
#ifdef VOIP_AEC_DEBUG_DUMP
  Voip_MoveReadPtr(aec->far_time_buf, move_elements);
#endif

  // 4) Process as many blocks as possible.
  while (Voip_available_read(aec->nearFrBuf) >= PART_LEN) {
    ProcessBlock(aec);
  }
  ...
  out_elements = (int)Voip_available_read(aec->outFrBuf);
  if (out_elements < FRAME_LEN) {
    Voip_MoveReadPtr(aec->outFrBuf, out_elements - FRAME_LEN);
    if (aec->sampFreq == 32000) {
      Voip_MoveReadPtr(aec->outFrBufH, out_elements - FRAME_LEN);
    }
  }
  // Obtain an output frame.
  Voip_ReadBuffer(aec->outFrBuf, NULL, out, FRAME_LEN);
  // For H band.
  if (aec->sampFreq == 32000) {
    Voip_ReadBuffer(aec->outFrBufH, NULL, outH, FRAME_LEN);
  }
}
...
int VoipAec_echo_state(AecCore* self) { return self->echoState; }
...
#ifdef VOIP_AEC_DEBUG_DUMP
void* VoipAec_far_time_buf(AecCore* self) { return self->far_time_buf; }
#endif
...
void VoipAec_enable_delay_correction(AecCore* self, int enable) {
  ...
  Voip_set_allowed_offset(self->delay_estimator, self->num_partitions / 2);
}
...
int VoipAec_system_delay(AecCore* self) { return self->system_delay; }
...
static void ProcessBlock(AecCore* aec) {
  ...
  if (aec->sampFreq == 32000) {
    ...
    Voip_ReadBuffer(aec->nearFrBufH, (void**)&nearend_ptr, nearend, PART_LEN);
    for (i = 0; i < PART_LEN; i++) {
      dH[i] = (float)(nearend_ptr[i]);
    }
    memcpy(aec->dBufH + PART_LEN, dH, sizeof(float) * PART_LEN);
  }
  Voip_ReadBuffer(aec->nearFrBuf, (void**)&nearend_ptr, nearend, PART_LEN);
...
#ifdef VOIP_AEC_DEBUG_DUMP
  {
    ...
    Voip_ReadBuffer(aec->far_time_buf, (void**)&farend_ptr, farend, 1);
  ...
  assert(Voip_available_read(aec->far_buf) > 0);
  Voip_ReadBuffer(aec->far_buf, (void**)&xf_ptr, &xf[0][0], 1);
  ...
  if (aec->delay_logging_enabled) {
    ...
    if (Voip_AddFarSpectrumFloat(
            aec->delay_estimator_farend, abs_far_spectrum, PART_LEN1) == 0) {
      delay_estimate = Voip_DelayEstimatorProcessFloat(
  ...
  VoipAec_ScaleErrorSignal(aec, ef);
  VoipAec_FilterAdaptation(aec, fft, ef);
  ...
  Voip_WriteBuffer(aec->outFrBuf, output, PART_LEN);
  // For H band
  if (aec->sampFreq == 32000) {
    Voip_WriteBuffer(aec->outFrBufH, outputH, PART_LEN);
  }
...
#ifdef VOIP_AEC_DEBUG_DUMP
  {
    ...
    for (i = 0; i < PART_LEN; i++) {
      eInt16[i] = (int16_t)VOIP_SPL_SAT(
          VOIP_SPL_WORD16_MAX, e[i], VOIP_SPL_WORD16_MIN);
    }
...
static void NonLinearProcessing(AecCore* aec, short* output, short* outputH) {
  ...
  // We should always have at least one element stored in |far_buf|.
  assert(Voip_available_read(aec->far_buf_windowed) > 0);
  // NLP
  Voip_ReadBuffer(aec->far_buf_windowed, (void**)&xfw_ptr, &xfw[0][0], 1);
  ...
  for (i = 0; i < PART_LEN1; i++) {
    ...
    aec->sx[i] =
        ptrGCoh[0] * aec->sx[i] +
        ptrGCoh[1] *
            VOIP_SPL_MAX(xfw[0][i] * xfw[0][i] + xfw[1][i] * xfw[1][i], 15);
  ...
  if (aec->hNlXdAvgMin == 1) {
    ...
    if (aec->stNearState == 1) {
      ...
      aec->echoState = 1;
      for (i = 0; i < PART_LEN1; i++) {
        hNl[i] = VOIP_SPL_MIN(cohde[i], 1 - cohxd[i]);
      }
  ...
  aec->hNlFbLocalMin =
      VOIP_SPL_MIN(aec->hNlFbLocalMin + 0.0008f / aec->mult, 1);
  aec->hNlXdAvgMin = VOIP_SPL_MIN(aec->hNlXdAvgMin + 0.0006f / aec->mult, 1);
  ...
  if (aec->hNlMinCtr == 2) {
    ...
    aec->hNlMinCtr = 0;
    aec->overDrive =
        VOIP_SPL_MAX(kTargetSupp[aec->nlp_mode] /
  ...
  VoipAec_OverdriveAndSuppress(aec, hNl, hNlFb, efw);

  // Add comfort noise.
  VoipAec_ComfortNoise(aec, efw, comfortNoiseHband, aec->noisePow, hNl);
  ...
  for (i = 0; i < PART_LEN; i++) {
    ...
    output[i] = (short)VOIP_SPL_SAT(
        VOIP_SPL_WORD16_MAX, fft[i], VOIP_SPL_WORD16_MIN);
  ...
  if (aec->sampFreq == 32000) {
    ...
    for (i = 0; i < PART_LEN; i++) {
      ...
      // Saturation protection
      outputH[i] = (short)VOIP_SPL_SAT(
          VOIP_SPL_WORD16_MAX, dtmp, VOIP_SPL_WORD16_MIN);
    }
  }
...
static void ComfortNoise(AecCore* aec,
  ...
  VoipSpl_RandUArray(randW16, PART_LEN, &aec->seed);
  for (i = 0; i < PART_LEN; i++) {
  ...
  for (i = 0; i < PART_LEN1; i++) {
    ...
    tmp = sqrtf(VOIP_SPL_MAX(1 - lambda[i] * lambda[i], 0));
    // tmp = 1 - lambda[i];
    efw[0][i] += tmp * u[i][0];
    efw[1][i] += tmp * u[i][1];
  }
  ...
  if (aec->sampFreq == 32000 && flagHbandCn == 1) {
    ...
    for (i = PART_LEN1 >> 1; i < PART_LEN1; i++) {
      num++;
      tmpAvg += sqrtf(VOIP_SPL_MAX(1 - lambda[i] * lambda[i], 0));
    }
    tmpAvg /= (float)num;
