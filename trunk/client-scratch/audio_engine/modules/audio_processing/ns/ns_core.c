...
#include "audio_engine/modules/audio_processing/ns/windows_private.h"
#include "audio_engine/modules/audio_processing/utility/fft4g.h"

// Set Feature Extraction Parameters
void VoipNs_set_feature_extraction_parameters(NSinst_t* inst) {
...
// Initialize state
int VoipNs_InitCore(NSinst_t* inst, uint32_t fs) {
  int i;
  ...
  Voip_rdft(inst->anaLen, 1, inst->dataBuf, inst->ip, inst->wfft);
  ...
  VoipNs_set_feature_extraction_parameters(inst); // Set feature configuration

  //default mode
  VoipNs_set_policy_core(inst, 0);
...
int VoipNs_set_policy_core(NSinst_t* inst, int mode) {
...
// Estimate noise
void VoipNs_NoiseEstimation(NSinst_t* inst, float* magn, float* noise) {
  int i, s, offset;
...
void VoipNs_FeatureParameterExtraction(NSinst_t* inst, int flag) {
...
void VoipNs_ComputeSpectralFlatness(NSinst_t* inst, float* magnIn) {
  int i;
...
void VoipNs_ComputeSpectralDifference(NSinst_t* inst, float* magnIn) {
...
void VoipNs_SpeechNoiseProb(NSinst_t* inst, float* probSpeechFinal, float* snrLocPrior,
                              float* snrLocPost) {
  int i, sgnMap;
...
int VoipNs_ProcessCore(NSinst_t* inst,
                         short* speechFrame,
                         short* speechFrameHB,
                         short* outFrame,
                         short* outFrameHB) {
  ...
  if (inst->outLen == 0) {
    ...
    if (energy1 == 0.0) {
      ...
      for (i = 0; i < inst->blockLen10ms; i++) {
        dTmp = fout[i];
        if (dTmp < VOIP_SPL_WORD16_MIN) {
          dTmp = VOIP_SPL_WORD16_MIN;
        } else if (dTmp > VOIP_SPL_WORD16_MAX) {
          dTmp = VOIP_SPL_WORD16_MAX;
        }
    ...
    // FFT
    Voip_rdft(inst->anaLen, 1, winData, inst->ip, inst->wfft);
    ...
    //compute spectral flatness on input spectrum
    VoipNs_ComputeSpectralFlatness(inst, magn);
    // quantile noise estimate
    VoipNs_NoiseEstimation(inst, magn, noise);
    ...
    VoipNs_ComputeSpectralDifference(inst, magn);
    ...
    if (updateParsFlag >= 1) {
      ...
      if (inst->modelUpdatePars[3] > 0) {
        VoipNs_FeatureParameterExtraction(inst, 0);
      }
      // compute model parameters
      if (inst->modelUpdatePars[3] == 0) {
        VoipNs_FeatureParameterExtraction(inst, 1);
        inst->modelUpdatePars[3] = inst->modelUpdatePars[1];
    ...
    // compute speech/noise probability
    VoipNs_SpeechNoiseProb(inst, probSpeechFinal, snrLocPrior, snrLocPost);
