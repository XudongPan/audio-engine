...
AudioDeviceBuffer::AudioDeviceBuffer() :
    ...
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "%s created", __FUNCTION__);
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, id, "AudioDeviceBuffer::SetId(id=%d)", id);
    _id = id;
}
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "AudioDeviceBuffer::SetRecordingSampleRate(fsHz=%u)", fsHz);
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "AudioDeviceBuffer::SetPlayoutSampleRate(fsHz=%u)", fsHz);
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "AudioDeviceBuffer::SetRecordingChannels(channels=%u)", channels);
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "AudioDeviceBuffer::SetPlayoutChannels(channels=%u)", channels);
                                             ...
                                             uint32_t nSamples)
{
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "invalid number of recorded samples (%d)", nSamples);
        return -1;
    }
...
int32_t AudioDeviceBuffer::DeliverRecordedData()
{
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to deliver recorded data (AudioTransport does not exist)");
        return 0;
    }
...
int32_t AudioDeviceBuffer::RequestPlayoutData(uint32_t nSamples)
{
    ...
    uint8_t playChannels = 0;
    {
        ...
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "invalid number of samples to be played out (%d)", nSamples);
            return -1;
        }
    }
    ...
    if (_ptrCbAudioTransport == NULL)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to feed data to playout (AudioTransport does not exist)");
        return 0;
    }
    ...
    if (_ptrCbAudioTransport)
    {
        ...
        if (res != 0)
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "NeedMorePlayData() failed");
        }
    }
...
int32_t AudioDeviceBuffer::GetPlayoutData(void* audioBuffer)
{
    ...
    if (_playSize > kMaxBufferSizeBytes)
    {
       VOIP_TRACE(kTraceError, kTraceUtility, _id, "_playSize %i exceeds "
