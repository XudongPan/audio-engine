#include "audio_engine/modules/audio_device/audio_device_config.h"
#include "audio_engine/modules/audio_device/audio_device_utility.h"
#include "audio_engine/modules/audio_device/win/audio_device_wave_win.h"
...
AudioDeviceWindowsWave::AudioDeviceWindowsWave(const int32_t id) :
    ...
    _maxMicVolume(0)
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, id, "%s created", __FUNCTION__);
...
{
    VOIP_TRACE(kTraceMemory, kTraceAudioDevice, _id, "%s destroyed", __FUNCTION__);
...
int32_t AudioDeviceWindowsWave::Init()
{
    ...
    const char* threadName = "voip_audio_module_thread";
    ...
    if (_ptrThread == NULL)
    {
        VOIP_TRACE(kTraceCritical, kTraceAudioDevice, _id,
    ...
    if (!_timeEvent.StartTimer(periodic, TIMER_PERIOD_MS))
    {
        ...
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id,
                         "unable to stop the activated thread");
        }
        return -1;
    }
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id,
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id,
...
int32_t AudioDeviceWindowsWave::SetWaveOutVolume(uint16_t volumeLeft, uint16_t volumeRight)
{
    ...
    if (_hWaveOut == NULL)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "no open playout device exists => using default");
    }
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutGetDevCaps() failed (err=%d)", res);
    ...
    if (!(caps.dwSupport & WAVECAPS_VOLUME))
    {
        ...
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "device does not support volume control using the Wave API");
        return -1;
    }
    if (!(caps.dwSupport & WAVECAPS_LRVOLUME))
    {
        // high-order word (right channel) is ignored
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "device does not support volume control on both channels");
    }
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveOutSetVolume() failed (err=%d)", res);
        TraceWaveOutError(res);
        return -1;
    }
...
int32_t AudioDeviceWindowsWave::WaveOutVolume(uint16_t& volumeLeft, uint16_t& volumeRight) const
{
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveOutGetVolume() failed (err=%d)", res);
        TraceWaveOutError(res);
        return -1;
    }
...
int32_t AudioDeviceWindowsWave::MicrophoneVolume(uint32_t& volume) const
{
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "failed to retrive current microphone level");
        return -1;
    }
...
int32_t AudioDeviceWindowsWave::SetPlayoutDevice(uint16_t index)
{
    ...
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "number of availiable waveform-audio output devices is %u", nDevices);

    if (index < 0 || index > (nDevices-1))
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "device index is out of range [0,%u]", (nDevices-1));
        return -1;
    }
    ...
    char guid[kAdmMaxGuidSize])
{
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutGetDevCapsW() failed (err=%d)", res);
        return -1;
    }
    if (WideCharToMultiByte(CP_UTF8, 0, caps.szPname, -1, name, kAdmMaxDeviceNameSize, NULL, NULL) == 0)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "WideCharToMultiByte(CP_UTF8) failed with error code %d - 1", GetLastError());
    }
    ...
    if (res != MMSYSERR_NOERROR)
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveOutMessage(DRV_QUERYFUNCTIONINSTANCEIDSIZE) failed (err=%d)", res);
        TraceWaveOutError(res);
        ...
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "WideCharToMultiByte(CP_UTF8) failed with error code %d - 2", GetLastError());
        }
        return 0;
    }
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveOutMessage(DRV_QUERYFUNCTIONINSTANCEID) failed (err=%d)", res);
        TraceWaveOutError(res);
        // Best we can do is to copy the friendly name and use it as guid
        if (WideCharToMultiByte(CP_UTF8, 0, caps.szPname, -1, guid, kAdmMaxGuidSize, NULL, NULL) == 0)
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "WideCharToMultiByte(CP_UTF8) failed with error code %d - 3", GetLastError());
        }
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "WideCharToMultiByte(CP_UTF8) failed with error code %d - 4", GetLastError());
    }
    CoTaskMemFree(pstrEndpointId);
    ...
    char guid[kAdmMaxGuidSize])
{
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInGetDevCapsW() failed (err=%d)", res);
        return -1;
    }
    if (WideCharToMultiByte(CP_UTF8, 0, caps.szPname, -1, name, kAdmMaxDeviceNameSize, NULL, NULL) == 0)
    {
    ...
    {
        // DRV_QUERYFUNCTIONINSTANCEIDSIZE is not supported <=> earlier version of Windows than Vista
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveInMessage(DRV_QUERYFUNCTIONINSTANCEIDSIZE) failed (err=%d)", res);
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveInMessage(DRV_QUERYFUNCTIONINSTANCEID) failed (err=%d)", res);
        TraceWaveInError(res);
        // Best we can do is to copy the friendly name and use it as guid
        if (WideCharToMultiByte(CP_UTF8, 0, caps.szPname, -1, guid, kAdmMaxGuidSize, NULL, NULL) == 0)
        {
...
int32_t AudioDeviceWindowsWave::SetRecordingDevice(uint16_t index)
{
    ...
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "number of availiable waveform-audio input devices is %u", nDevices);
...
int32_t AudioDeviceWindowsWave::InitPlayout()
{
    ...
    if (InitSpeaker() == -1)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "InitSpeaker() failed");
    }
    ...
    if (_hWaveOut != NULL)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutClose() failed (err=%d)", res);
            TraceWaveOutError(res);
        }
    }
    ...
    if (IsUsingOutputDeviceIndex())
    {
        ...
        if (MMSYSERR_NOERROR == res)
        {
            ...
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "opening output device corresponding to device ID %u", _outputDeviceIndex);
        }
    }
    else
    {
        if (_outputDevice == AudioDeviceModule::kDefaultCommunicationDevice)
        {
            ...
            if (MMSYSERR_NOERROR == res)
            {
                ...
                VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "opening default communication device");
            }
            else
            {
                ...
                VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "unable to open default communication device => using default instead");
            }
        }
        else if (_outputDevice == AudioDeviceModule::kDefaultDevice)
        {
            ...
            {
                res = waveOutOpen(&hWaveOut, WAVE_MAPPER, &waveFormat, 0, 0, CALLBACK_NULL);
                VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "opening default output device");
            }
        }
    }
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveOutOpen() failed (err=%d)", res);
        TraceWaveOutError(res);
        return -1;
    }
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutGetID() failed (err=%d)", res);
        TraceWaveOutError(res);
    }
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "utilized device ID : %u", deviceID);
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "product name       : %s", caps.szPname);
    ...
    for (int n = 0; n < N_BUFFERS_OUT; n++)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutPrepareHeader(%d) failed (err=%d)", n, res);
            TraceWaveOutError(res);
        }
        ...
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutPrepareHeader(%d) failed (dwFlags != WHDR_PREPARED)", n);
        }
    }
    ...
    VOIP_TRACE(kTraceInfo, kTraceUtility, _id,"initial playout status: _playBufDelay=%d, _minPlayBufDelay=%d",
...
int32_t AudioDeviceWindowsWave::InitRecording()
{
    ...
    if (InitMicrophone() == -1)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "InitMicrophone() failed");
    }
    ...
    if (_hWaveIn != NULL)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInClose() failed (err=%d)", res);
            TraceWaveInError(res);
        }
    }
    ...
    if (IsUsingInputDeviceIndex())
    {
        ...
        if (MMSYSERR_NOERROR == res)
        {
            ...
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "opening input device corresponding to device ID %u", _inputDeviceIndex);
        }
    }
    else
    {
        ...
        else if (_inputDevice == AudioDeviceModule::kDefaultDevice)
        {
            ...
            {
                res = waveInOpen(&hWaveIn, WAVE_MAPPER, &waveFormat, 0, 0, CALLBACK_NULL);
                VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "opening default input device");
            }
        }
    }
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveInOpen() failed (err=%d)", res);
        TraceWaveInError(res);
        return -1;
    }
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInGetDevCaps() failed (err=%d)", res);
        TraceWaveInError(res);
    }
    ...
    if (res != MMSYSERR_NOERROR)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInGetID() failed (err=%d)", res);
        TraceWaveInError(res);
    }
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "utilized device ID : %u", deviceID);
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "product name       : %s", caps.szPname);
...
int32_t AudioDeviceWindowsWave::StartRecording()
{
    ...
    if (kEventTimeout == _recStartEvent.Wait(10000))
    {
        _startRec = false;
        StopRecording();
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to activate recording");
        return -1;
    }
...
int32_t AudioDeviceWindowsWave::StopRecording()
{
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInStop() failed (err=%d)", res);
        TraceWaveInError(res);
    }
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInReset() failed (err=%d)", res);
        TraceWaveInError(res);
    }
    ...
    if (wasRecording)
    {
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "waveInUnprepareHeader() will be performed");
        for (int n = 0; n < N_BUFFERS_IN; n++)
        {
            res = waveInUnprepareHeader(_hWaveIn, &_waveHeaderIn[n], sizeof(WAVEHDR));
            if (MMSYSERR_NOERROR != res)
            {
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInUnprepareHeader() failed (err=%d)", res);
                TraceWaveInError(res);
            }
        }
    }
    ...
    //
    _hWaveIn = NULL;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "_hWaveIn is now set to NULL");
...
int32_t AudioDeviceWindowsWave::StartPlayout()
{
    ...
    if (kEventTimeout == _playStartEvent.Wait(10000))
    {
        _startPlay = false;
        StopPlayout();
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to activate playout");
        return -1;
    }
    ...
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "failed to activate playing");
        return -1;
    }
...
int32_t AudioDeviceWindowsWave::StopPlayout()
{
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutReset() failed (err=%d)", res);
        TraceWaveOutError(res);
    }
    ...
    for (int n = 0; n < N_BUFFERS_OUT; n++)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutUnprepareHeader() failed (err=%d)", res);
            TraceWaveOutError(res);
        }
    }
    ...
    _hWaveOut = NULL;
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "_hWaveOut is now set to NULL");
...
int32_t AudioDeviceWindowsWave::InputSanityCheckAfterUnlockedPeriod() const
{
    if (_hWaveIn == NULL)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "input state has been modified during unlocked period");
        return -1;
    }
    return 0;
}
...
int32_t AudioDeviceWindowsWave::OutputSanityCheckAfterUnlockedPeriod() const
{
    if (_hWaveOut == NULL)
    {
        VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "output state has been modified during unlocked period");
        return -1;
    }
    return 0;
}
...
int32_t AudioDeviceWindowsWave::EnumeratePlayoutDevices()
{
    ...
    uint16_t nDevices(PlayoutDevices());
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "===============================================================");
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "#output devices: %u", nDevices);
    ...
    for (UINT deviceID = 0; deviceID < nDevices; deviceID++)
    {
        ...
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "===============================================================");
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "Device ID %u:", deviceID);
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "manufacturer ID      : %u", caps.wMid);
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "product ID           : %u",caps.wPid);
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "version of driver    : %u.%u", HIBYTE(caps.vDriverVersion), LOBYTE(caps.vDriverVersion));
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "product name         : %s", caps.szPname);
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "dwFormats            : 0x%x", caps.dwFormats);
        if (caps.dwFormats & WAVE_FORMAT_48S16)
        {
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "  48kHz,stereo,16bit : SUPPORTED");
        }
        else
        {
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, " 48kHz,stereo,16bit  : *NOT* SUPPORTED");
        }
        if (caps.dwFormats & WAVE_FORMAT_48M16)
        {
            VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "  48kHz,mono,16bit   : SUPPORTED");
        }
        else
        {
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, " 48kHz,mono,16bit    : *NOT* SUPPORTED");
        }
        VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "wChannels            : %u", caps.wChannels);
...
int32_t AudioDeviceWindowsWave::EnumerateRecordingDevices()
{
    ...
    uint16_t nDevices(RecordingDevices());
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "===============================================================");
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "#input devices: %u", nDevices);
...
void AudioDeviceWindowsWave::TraceSupportFlags(DWORD dwSupport) const
{
    ...
    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "%S", buf);
}
...
int32_t AudioDeviceWindowsWave::PrepareStartRecording()
{
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInGetPosition(TIME_SAMPLES) failed (err=%d)", res);
        TraceWaveInError(res);
    }
    ...
    for (int n = 0; n < N_BUFFERS_IN; n++)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInPrepareHeader(%d) failed (err=%d)", n, res);
            TraceWaveInError(res);
        }
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInAddBuffer(%d) failed (err=%d)", n, res);
            TraceWaveInError(res);
        }
    }
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInStart() failed (err=%d)", res);
        TraceWaveInError(res);
    }
...
int32_t AudioDeviceWindowsWave::GetPlayoutBufferDelay(uint32_t& writtenSamples, uint32_t& playedSamples)
{
    ...
    if (MMSYSERR_NOERROR != res)
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveOutGetPosition() failed (err=%d)", res);
        TraceWaveOutError(res);
    }
    ...
    if (playedDifference > 64000)
    {
        ...
        if((i < 31) && (i > 14)) {
            ...
            //
            VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "msecleft() => wrap around occured: %d bits used by sound card)", (i+1));
    ...
    else if ((_writtenSamplesOld > POW2(31)) && (writtenSamples < 96000))
    {
        ...
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "  msecleft() (wrap around occured after having used all 32 bits)");
    ...
    else if ((writtenSamples < 96000) && (playedSamples > POW2(31)))
    {
        ...
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "  msecleft() (wrap around occured: correction of output is done)");
    ...
    if (_dc_diff_mean < -200)
    {
        ...
        if (_useHeader == 1)
        {
            ...
            VOIP_TRACE(kTraceInfo, kTraceUtility, -1, "Modification #1: _useHeader = %d, _minPlayBufDelay = %d", _useHeader, _minPlayBufDelay);
        }
        else if (_useHeader == 2)
        {
            _minPlayBufDelay = 100;   // add some more safety
            VOIP_TRACE(kTraceInfo, kTraceUtility, -1, "Modification #2: _useHeader = %d, _minPlayBufDelay = %d", _useHeader, _minPlayBufDelay);
        }
        else
        {
            // This state should not be entered... (HA)
            VOIP_TRACE (kTraceWarning, kTraceUtility, -1, "further actions are required!");
        }
        if (_playWarning == 1)
        {
            VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "pending playout warning exists");
        }
        _playWarning = 1;  // triggers callback from module process thread
        VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "kPlayoutWarning message posted: switching to alternative playout delay method");
    }
    _dc_prevtime = time;
    ...
    if ((ms_Header-50) > msecInPlayoutBuffer) {
        ...
        if (_no_of_msecleft_warnings%20==0)
        {
            ...
            VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "%S", infoStr);
        }
    ...
    if (ms_Header < msecInPlayoutBuffer)
    {
        if (_no_of_msecleft_warnings % 100 == 0)
        {
            ...
            VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "%S", str);
        }
        _no_of_msecleft_warnings++;
...
int32_t AudioDeviceWindowsWave::GetRecordingBufferDelay(uint32_t& readSamples, uint32_t& recSamples)
{
    ...
    {
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInGetPosition() failed (err=%d)", mmr);
    ...
    if( recDifference > 64000) {
        VOIP_TRACE (kTraceDebug, kTraceUtility, -1,"WRAP 1 (recDifference =%d)", recDifference);
        ...
        if((i < 31) && (i > 14)) {
            ...
            VOIP_TRACE (kTraceWarning, kTraceUtility, -1,"AEC (_rec_samples_old %d recSamples %d)",_rec_samples_old, recSamples);
        }
    }
    ...
    if((_wrapCounter>200)){
        ...
        VOIP_TRACE (kTraceDebug, kTraceUtility, -1,"WRAP 2 (_rec_samples_old %d recSamples %d)",_rec_samples_old, recSamples);
        ...
        VOIP_TRACE (kTraceDebug, kTraceUtility, -1,"WRAP 3 (readSamples %d recSamples %d)",readSamples, recSamples);
    ...
    if((res > 2000)||(res < 0)||(_wrapCounter>200)){
        // Reset everything
        VOIP_TRACE (kTraceWarning, kTraceUtility, -1,"msec_read error (res %d wrapCounter %d)",res, _wrapCounter);
        MMTIME mmtime;
        mmtime.wType = TIME_SAMPLES;

        mmr=waveInGetPosition(_hWaveIn, &mmtime, sizeof(mmtime));
        if (mmr != MMSYSERR_NOERROR) {
            VOIP_TRACE (kTraceWarning, kTraceUtility, -1, "waveInGetPosition failed (mmr=%d)", mmr);
        }
        _read_samples=mmtime.u.sample;
        _read_samples_old=_read_samples;
        _rec_samples_old=mmtime.u.sample;
...
bool AudioDeviceWindowsWave::ThreadProcess()
{
    ...
    case kEventSignaled:
        break;
    case kEventError:
        VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "EventWrapper::Wait() failed => restarting timer");
        _timeEvent.StopTimer();
        ...
        (playDiff < 0))
    {
        ...
        if (_playing)
        {
            ...
            {
                VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "PlayProc() failed");
            }
            _prevPlayTime = time;
            if (playTime != 0)
                _playAcc += playTime;
        ...
        if (_recording)
        {
            ...
            while ((nRecordedBytes = RecProc(recTime)) > 0)
            {
                ...
                if (maxIter == 0)
                {
                    ...
                    VOIP_TRACE(kTraceInfo, kTraceAudioDevice, _id, "failed to compensate for reduced MM-timer resolution");
                }
            }

            if (nRecordedBytes == -1)
            {
                VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "RecProc() failed");
            }
...
int32_t AudioDeviceWindowsWave::RecProc(LONGLONG& consumedTime)
{
       ...
       (nBytesRecorded > 0))
    {
        ...
        else
        {
            ...
            {
                VOIP_TRACE(kTraceDebug, kTraceUtility, _id,"nBytesRecorded=%d => don't use", nBytesRecorded);
                _timesdwBytes = 0;
                send = false;
            }
        }
        ...
        // Store the play and rec delay values for  synchronization
        ...
        if (_AGC)
        {
            ...
            if (newMicLevel != 0)
            {
                ...
                VOIP_TRACE(kTraceStream, kTraceUtility, _id,"AGC change of volume: => new=%u", newMicLevel);
        ...
        if (_recDelayCount > (_recPutBackDelay-1))
        {
            ...
            if (MMSYSERR_NOERROR != res)
            {
                VOIP_TRACE(kTraceWarning, kTraceAudioDevice, _id, "waveInUnprepareHeader(%d) failed (err=%d)", bufCount, res);
                TraceWaveInError(res);
            }
            ...
            if (res != MMSYSERR_NOERROR)
            {
                VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveInPrepareHeader(%d) failed (err=%d)", bufCount, res);
                TraceWaveInError(res);
                return -1;
            }
            ...
            if (res != MMSYSERR_NOERROR)
            {
                VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveInAddBuffer(%d) failed (err=%d)", bufCount, res);
                TraceWaveInError(res);
                if (_recPutBackDelay < 50)
                {
                    _recPutBackDelay++;
                    VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "_recPutBackDelay increased to %d", _recPutBackDelay);
                }
                else
                {
                    if (_recError == 1)
                    {
                        VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "pending recording error exists");
                    }
                    _recError = 1;  // triggers callback from module process thread
                    VOIP_TRACE(kTraceError, kTraceUtility, _id, "kRecordingError message posted: _recPutBackDelay=%u", _recPutBackDelay);
                }
            }
        }  // if (_recDelayCount > (_recPutBackDelay-1))
...
int AudioDeviceWindowsWave::PlayProc(LONGLONG& consumedTime)
{
    ...
    if (remTimeMS < thresholdMS + 9)
    {
        ...
        if (remTimeMS == 0)
        {
            VOIP_TRACE(kTraceInfo, kTraceUtility, _id, "playout buffer is empty => we must adapt...");
            if (_waitCounter > 30)
            {
                _erZeroCounter++;
                if (_erZeroCounter == 2)
                {
                    ...
                    _minPlayBufDelay += 20;
                    _waitCounter = 50;
                    VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "New playout states (er=0,erZero=2): minPlayBufDelay=%u, playBufDelay=%u", _minPlayBufDelay, _playBufDelay);
                }
                else if (_erZeroCounter == 3)
                {
                    ...
                    _minPlayBufDelay += 25;
                    _waitCounter = 0;
                    VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "New playout states (er=0, erZero=3): minPlayBufDelay=%u, playBufDelay=%u", _minPlayBufDelay, _playBufDelay);
                }
                else
                {
                    _minPlayBufDelay += 10;
                    _playBufDelay += 15;
                    _waitCounter = 50;
                    VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "New playout states (er=0, erZero=1): minPlayBufDelay=%u, playBufDelay=%u", _minPlayBufDelay, _playBufDelay);
                }
            }
        }
        else if (remTimeMS < _minPlayBufDelay)
        {
            ...
            if (_waitCounter > 30)
            {
                ...
                if (_intro == 0)
                    _waitCounter = 0;
                VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Playout threshold is increased: playBufDelay=%u", _playBufDelay);
            }
        }
        ...
        {
            VOIP_TRACE(kTraceError, kTraceUtility, _id, "invalid number of output samples(%d)", nSamples);
        }
    ...
    else if (thresholdMS + 9 < remTimeMS )
    {
        ...
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Need to check playout buffer more often (dT=%u, remTimeMS=%u)", _dTcheckPlayBufDelay, remTimeMS);
    }
    ...
    if (_waitCounter > 2000)
    {
        ...
        _playBufDelay--;
        _waitCounter = 1990;
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Playout threshold is decreased: playBufDelay=%u", _playBufDelay);
    }
    ...
    {
        _playBufDelay = _minPlayBufDelay;
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Playout threshold is limited to %u", _minPlayBufDelay);
    }
    ...
    {
        _playBufDelay = 150;
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Playout threshold is limited to %d", _playBufDelay);
    }
       ...
       (_useHeader == 0))
    {
        ...
        VOIP_TRACE(kTraceDebug, kTraceUtility, _id, "Minimum playout threshold is limited to %d", _MAX_minBuffer);
    }
...
int32_t AudioDeviceWindowsWave::Write(int8_t* data, uint16_t nSamples)
{
    ...
    if (_playIsInitialized)
    {
        ...
        if (MMSYSERR_NOERROR != res)
        {
            VOIP_TRACE(kTraceError, kTraceAudioDevice, _id, "waveOutWrite(%d) failed (err=%d)", bufCount, res);
            TraceWaveOutError(res);
            ...
            if (_writeErrors > 10)
            {
                if (_playError == 1)
                {
                    VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "pending playout error exists");
                }
                _playError = 1;  // triggers callback from module process thread
                VOIP_TRACE(kTraceError, kTraceUtility, _id, "kPlayoutError message posted: _writeErrors=%u", _writeErrors);
            }
...
int32_t AudioDeviceWindowsWave::MonitorRecording(const uint32_t time)
{
    ...
    if (nRecordedSamples > 5*N_REC_SAMPLES_PER_SEC)
    {
        ...
        if ((time - _prevRecByteCheckTime) > 5700)
        {
            ...
            if (_recWarning == 1)
            {
                VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "pending recording warning exists");
            }
            _recWarning = 1;  // triggers callback from module process thread
            VOIP_TRACE(kTraceWarning, kTraceUtility, _id, "kRecordingWarning message posted: time-_prevRecByteCheckTime=%d", time - _prevRecByteCheckTime);
        }
    ...
    if ((time - _prevRecByteCheckTime) > 8000)
    {
        ...
        _recError = 1;  // triggers callback from module process thread
        VOIP_TRACE(kTraceError, kTraceUtility, _id, "kRecordingError message posted: time-_prevRecByteCheckTime=%d", time - _prevRecByteCheckTime);
...
int32_t AudioDeviceWindowsWave::RestartTimerIfNeeded(const uint32_t time)
{
    ...
    if (diffMS > 7)
    {
        ...
        if (_timerFaults > 5 && _timerRestartAttempts < 2)
        {
            ...
            VOIP_TRACE(kTraceWarning, kTraceUtility, _id, " timer issue detected => timer is restarted");
            _timeEvent.StopTimer();
            _timeEvent.StartTimer(true, TIMER_PERIOD_MS);
