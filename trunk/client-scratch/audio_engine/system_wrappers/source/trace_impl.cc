...
#ifdef _WIN32
#include "audio_engine/system_wrappers/source/trace_win.h"
#else
#include "audio_engine/system_wrappers/source/trace_posix.h"
#endif  // _WIN32
...
TraceImpl::TraceImpl()
  ...
  for (int m = 0; m < VOIP_TRACE_NUM_ARRAY; ++m) {
    for (int n = 0; n < VOIP_TRACE_MAX_QUEUE; ++n) {
      message_queue_[m][n] = new
      char[VOIP_TRACE_MAX_MESSAGE_SIZE];
    }
  }
}
  ...
  if (idl != -1) {
    ...
    switch (module) {
      ...
      case kTraceVoice:
        ...
        break;
      case kTraceVData:
        sprintf(trace_message, "       VDATA:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceUtility:
        sprintf(trace_message, "     UTILITY:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceAudioPacket:
        sprintf(trace_message, "    AUDIO_PACKET:%5ld %5ld;", id_engine,
                id_channel);
        break;
      ...
      case kTraceAudioCoding:
        sprintf(trace_message, "AUDIO CODING:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceStreamAudioPacket:
        sprintf(trace_message, "        STREAM_AUDIO_PACKET:%5ld %5ld;", id_engine,
                id_channel);
        break;
      ...
      case kTraceAudioMixerClient:
        sprintf(trace_message, " AUDIO MIX/C:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceVDataCoding:
        sprintf(trace_message, "VDATA CODING:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceVDataMixer:
        // Print sleep time and API call
        sprintf(trace_message, "   VDATA MIX:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceFile:
      ...
      case kTraceAudioDevice:
        sprintf(trace_message, "AUDIO DEVICE:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceVDataRenderer:
        sprintf(trace_message, "VDATA RENDER:%5ld %5ld;", id_engine,
                id_channel);
        break;
      case kTraceVDataCapture:
        sprintf(trace_message, "VDATA CAPTUR:%5ld %5ld;", id_engine,
                id_channel);
        break;
    ...
    switch (module) {
      ...
      case kTraceVoice:
        ...
        break;
      case kTraceVData:
        sprintf(trace_message, "       VDATA:%11ld;", idl);
        break;
      case kTraceUtility:
        sprintf(trace_message, "     UTILITY:%11ld;", idl);
        break;
      case kTraceAudioPacket:
        sprintf(trace_message, "    AUDIO_PACKET:%11ld;", idl);
        break;
      case kTraceTransport:
      ...
      case kTraceAudioCoding:
        ...
        break;
      case kTraceStreamAudioPacket:
        sprintf(trace_message, "        STREAM_AUDIO_PACKET:%11ld;", idl);
        break;
      case kTraceAudioMixerServer:
      ...
      case kTraceAudioMixerClient:
        ...
        break;
      case kTraceVDataCoding:
        sprintf(trace_message, "VDATA CODING:%11ld;", idl);
        break;
      case kTraceVDataMixer:
        sprintf(trace_message, "   VDATA MIX:%11ld;", idl);
        break;
      case kTraceFile:
      ...
      case kTraceAudioDevice:
        ...
        break;
      case kTraceVDataRenderer:
        sprintf(trace_message, "VDATA RENDER:%11ld;", idl);
        break;
      case kTraceVDataCapture:
        sprintf(trace_message, "VDATA CAPTUR:%11ld;", idl);
        break;
      case kTraceRemoteBitrateEstimator:
...
int32_t TraceImpl::AddMessage(
  ...
  int length = 0;
  if (written_so_far >= VOIP_TRACE_MAX_MESSAGE_SIZE) {
    return -1;
  }
  // - 2 to leave room for newline and NULL termination.
#ifdef _WIN32
  length = _snprintf(trace_message,
                     VOIP_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2,
                     "%s", msg);
  if (length < 0) {
    length = VOIP_TRACE_MAX_MESSAGE_SIZE - written_so_far - 2;
...
// NOTE(andresp): Enabled externally.
  ...
  if (next_free_idx_[active_queue_] >= VOIP_TRACE_MAX_QUEUE) {
    if (!trace_file_.Open() && !callback_) {
      ...
      //                that?
      for (int n = 0; n < VOIP_TRACE_MAX_QUEUE / 4; ++n) {
        const int last_quarter_offset = (3 * VOIP_TRACE_MAX_QUEUE / 4);
        memcpy(message_queue_[active_queue_][n],
               message_queue_[active_queue_][n + last_quarter_offset],
               VOIP_TRACE_MAX_MESSAGE_SIZE);
      }
      next_free_idx_[active_queue_] = VOIP_TRACE_MAX_QUEUE / 4;
    } else {
  ...
  if (next_free_idx_[active_queue_] == VOIP_TRACE_MAX_QUEUE - 1) {
...
void TraceImpl::WriteToFile() {
  ...
  for (uint16_t idx = 0; idx < local_next_free_idx; ++idx) {
    ...
    if (trace_file_.Open()) {
      if (row_count_text_ > VOIP_TRACE_MAX_FILE_SIZE) {
        // wrap file
        row_count_text_ = 0;
        trace_file_.Flush();
      ...
      if (row_count_text_ ==  0) {
        char message[VOIP_TRACE_MAX_MESSAGE_SIZE + 1];
...
void TraceImpl::AddImpl(const TraceLevel level, const TraceModule module,
                        ...
                        const char msg[VOIP_TRACE_MAX_MESSAGE_SIZE]) {
  if (TraceCheck(level)) {
    char trace_message[VOIP_TRACE_MAX_MESSAGE_SIZE];
...
void Trace::Add(const TraceLevel level, const TraceModule module,
  ...
  if (trace) {
    if (trace->TraceCheck(level)) {
      char temp_buff[VOIP_TRACE_MAX_MESSAGE_SIZE];
      char* buff = 0;
      if (msg) {
        va_list args;
        va_start(args, msg);
#ifdef _WIN32
        _vsnprintf(temp_buff, VOIP_TRACE_MAX_MESSAGE_SIZE - 1, msg, args);
#else
        vsnprintf(temp_buff, VOIP_TRACE_MAX_MESSAGE_SIZE - 1, msg, args);
#endif
        va_end(args);
        buff = temp_buff;
      }
