...
static void HighPassFilter(const int16_t* data_in, int data_length,
  ...
  for (i = 0; i < data_length; i++) {
    ...
    tmp32 = VOIP_SPL_MUL_16_16(kHpZeroCoefs[0], *in_ptr);
    tmp32 += VOIP_SPL_MUL_16_16(kHpZeroCoefs[1], filter_state[0]);
    tmp32 += VOIP_SPL_MUL_16_16(kHpZeroCoefs[2], filter_state[1]);
    filter_state[1] = filter_state[0];
    filter_state[0] = *in_ptr++;

    // All-pole section (filter coefficients in Q14).
    tmp32 -= VOIP_SPL_MUL_16_16(kHpPoleCoefs[1], filter_state[2]);
    tmp32 -= VOIP_SPL_MUL_16_16(kHpPoleCoefs[2], filter_state[3]);
...
static void AllPassFilter(const int16_t* data_in, int data_length,
  ...
  for (i = 0; i < data_length; i++) {
    tmp32 = state32 + VOIP_SPL_MUL_16_16(filter_coefficient, *data_in);
    tmp16 = (int16_t) (tmp32 >> 16);  // Q(-1)
    *data_out++ = tmp16;
    state32 = (((int32_t) (*data_in)) << 14); // Q14
    state32 -= VOIP_SPL_MUL_16_16(filter_coefficient, tmp16);  // Q14
    state32 <<= 1;  // Q15.
    data_in += 2;
  }
...
static void LogOfEnergy(const int16_t* data_in, int data_length,
  ...
  energy = (uint32_t) VoipSpl_Energy((int16_t*) data_in, data_length,
                                       &tot_rshifts);

  if (energy != 0) {
    // By construction, normalizing to 15 bits is equivalent with 17 leading
    // zeros of an unsigned 32 bit value.
    int normalizing_rshifts = 17 - VoipSpl_NormU32(energy);
    ...
    *log_energy = (int16_t) (VOIP_SPL_MUL_16_16_RSFT(
        kLogConst, log2_energy, 19) +
        VOIP_SPL_MUL_16_16_RSFT(tot_rshifts, kLogConst, 9));
  ...
  *log_energy += offset;
  ...
  // energy indicator in VoIPVad_GmmProbability() in vad_core.c.
