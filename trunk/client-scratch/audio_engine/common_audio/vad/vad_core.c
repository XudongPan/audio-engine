...
#include "audio_engine/common_audio/vad/vad_filterbank.h"
#include "audio_engine/common_audio/vad/vad_gmm.h"
#include "audio_engine/common_audio/vad/vad_sp.h"
...
// Constants in VoIPVad_InitCore().
// Default aggressiveness mode.
static const short kDefaultMode = 0;
static const int kInitCheck = 42;

// Constants used in VoIPVad_set_mode_core().
//
  ...
  if (total_power > kMinEnergy) {
    ...
    for (channel = 0; channel < kNumChannels; channel++) {
      ...
      for (k = 0; k < kNumGaussians; k++) {
        ...
        tmp1_s32 = VoipVad_GaussianProbability(features[channel],
      ...
      // cancel.
      shifts_h0 = VoipSpl_NormW32(h0_test);
      shifts_h1 = VoipSpl_NormW32(h1_test);
      if (h0_test == 0) {
        shifts_h0 = 31;
      }
      if (h1_test == 0) {
      ...
      if (h0 > 0) {
        ...
        ngprvec[channel] = (int16_t) VoipSpl_DivW32W16(tmp1_s32, h0);  // Q14
      ...
      if (h1 > 0) {
        ...
        sgprvec[channel] = (int16_t) VoipSpl_DivW32W16(tmp1_s32, h1);  // Q14
    ...
    for (channel = 0; channel < kNumChannels; channel++) {
      ...
      feature_minimum = VoipVad_FindMinimum(self, features[channel], channel);
      ...
      for (k = 0; k < kNumGaussians; k++) {
        ...
        if (!vadflag) {
          ...
          // (Q14 * Q11 >> 11) = Q14.
          delt = (int16_t) VOIP_SPL_MUL_16_16_RSFT(ngprvec[gaussian],
                                                     deltaN[gaussian],
                                                     11);
          // Q7 + (Q14 * Q15 >> 22) = Q7.
          nmk2 = nmk + (int16_t) VOIP_SPL_MUL_16_16_RSFT(delt,
                                                           kNoiseUpdateConst,
                                                           22);
        }
        ...
        // Q7 + (Q8 * Q8) >> 9 = Q7.
        nmk3 = nmk2 + (int16_t) VOIP_SPL_MUL_16_16_RSFT(ndelt, kBackEta, 9);
        ...
        if (vadflag) {
          ...
          // (Q14 * Q11) >> 11 = Q14.
          delt = (int16_t) VOIP_SPL_MUL_16_16_RSFT(sgprvec[gaussian],
                                                     deltaS[gaussian],
                                                     11);
          // Q14 * Q15 >> 21 = Q8.
          tmp_s16 = (int16_t) VOIP_SPL_MUL_16_16_RSFT(delt,
                                                        kSpeechUpdateConst,
                                                        21);
          ...
          // (Q11 * Q4 >> 3) = Q12.
          tmp1_s32 = VOIP_SPL_MUL_16_16_RSFT(deltaS[gaussian], tmp_s16, 3);
          ...
          // 0.1 * Q20 / Q7 = Q13.
          if (tmp2_s32 > 0) {
            tmp_s16 = (int16_t) VoipSpl_DivW32W16(tmp2_s32, ssk * 10);
          } else {
            tmp_s16 = (int16_t) VoipSpl_DivW32W16(-tmp2_s32, ssk * 10);
            tmp_s16 = -tmp_s16;
          }
          ...
          // (Q11 * Q4 >> 3) = Q12.
          tmp1_s32 = VOIP_SPL_MUL_16_16_RSFT(deltaN[gaussian], tmp_s16, 3);
          tmp1_s32 -= 4096;
          ...
          // Q20 / Q7 = Q13.
          if (tmp1_s32 > 0) {
            tmp_s16 = (int16_t) VoipSpl_DivW32W16(tmp1_s32, nsk);
          } else {
            tmp_s16 = (int16_t) VoipSpl_DivW32W16(-tmp1_s32, nsk);
            tmp_s16 = -tmp_s16;
          }
      ...
      if (diff < kMinimumDifference[channel]) {
        ...
        tmp1_s16 = (int16_t) VOIP_SPL_MUL_16_16_RSFT(13, tmp_s16, 2);
        tmp2_s16 = (int16_t) VOIP_SPL_MUL_16_16_RSFT(3, tmp_s16, 2);
...
int VoipVad_InitCore(VadInstT* self) {
  int i;
  ...
  VoipSpl_ResetResample48khzTo8khz(&self->state_48_to_8);
  ...
  // Initialize mean value memory, for VoIPVad_FindMinimum().
  for (i = 0; i < kNumChannels; i++) {
    self->mean_value[i] = 1600;
  }

  // Set aggressiveness mode to default (=|kDefaultMode|).
  if (VoipVad_set_mode_core(self, kDefaultMode) != 0) {
    return -1;
  }
...
int VoipVad_set_mode_core(VadInstT* self, int mode) {
  int return_value = 0;
...
int VoipVad_CalcVad48khz(VadInstT* inst, int16_t* speech_frame,
                           int frame_length) {
  int vad;
  int i;
  ...
  for (i = 0; i < num_10ms_frames; i++) {
    VoipSpl_Resample48khzTo8khz(speech_frame,
  ...
  vad = VoipVad_CalcVad8khz(inst, speech_nb, frame_length / 6);

  return vad;
}

int VoipVad_CalcVad32khz(VadInstT* inst, int16_t* speech_frame,
                           int frame_length)
{
    int len, vad;
    ...
    VoipVad_Downsampling(speech_frame, speechWB, &(inst->downsampling_filter_states[2]),
                           frame_length);
    len = VOIP_SPL_RSHIFT_W16(frame_length, 1);

    VoipVad_Downsampling(speechWB, speechNB, inst->downsampling_filter_states, len);
    len = VOIP_SPL_RSHIFT_W16(len, 1);

    // Do VAD on an 8 kHz signal
    vad = VoipVad_CalcVad8khz(inst, speechNB, len);

    return vad;
}

int VoipVad_CalcVad16khz(VadInstT* inst, int16_t* speech_frame,
                           int frame_length)
{
    int len, vad;
    int16_t speechNB[240]; // Downsampled speech frame: 480 samples (30ms in WB)
    ...
    VoipVad_Downsampling(speech_frame, speechNB, inst->downsampling_filter_states,
                           frame_length);
...
int VoipVad_CalcVad8khz(VadInstT* inst, int16_t* speech_frame,
                          int frame_length)
{
    int16_t feature_vector[kNumChannels], total_power;

    // Get power in the bands
    total_power = VoipVad_CalculateFeatures(inst, speech_frame, frame_length,
                                              feature_vector);
