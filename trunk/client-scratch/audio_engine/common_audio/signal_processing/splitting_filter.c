...
static const uint16_t VoipSpl_kAllPassFilter1[3] = {6418, 36982, 57261};
static const uint16_t VoipSpl_kAllPassFilter2[3] = {21333, 49062, 63010};
...
void VoipSpl_AllPassQMF(int32_t* in_data, int16_t data_length,
                          ...
                          int32_t* filter_state)
{
    ...
    diff = VOIP_SPL_SUB_SAT_W32(in_data[0], filter_state[1]); // = (x[0] - y_1[-1])
    // y_1[0] =  x[-1] + a_1 * (x[0] - y_1[-1])
    out_data[0] = VOIP_SPL_SCALEDIFF32(filter_coefficients[0], diff, filter_state[0]);

    // For the remaining loops, use previous values.
    for (k = 1; k < data_length; k++)
    {
        diff = VOIP_SPL_SUB_SAT_W32(in_data[k], out_data[k - 1]); // = (x[n] - y_1[n-1])
        // y_1[n] =  x[n-1] + a_1 * (x[n] - y_1[n-1])
        out_data[k] = VOIP_SPL_SCALEDIFF32(filter_coefficients[0], diff, in_data[k - 1]);
    }
    ...
    diff = VOIP_SPL_SUB_SAT_W32(out_data[0], filter_state[3]); // = (y_1[0] - y_2[-1])
    // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
    in_data[0] = VOIP_SPL_SCALEDIFF32(filter_coefficients[1], diff, filter_state[2]);
    for (k = 1; k < data_length; k++)
    {
        diff = VOIP_SPL_SUB_SAT_W32(out_data[k], in_data[k - 1]); // =(y_1[n] - y_2[n-1])
        // y_2[0] =  y_1[-1] + a_2 * (y_1[0] - y_2[-1])
        in_data[k] = VOIP_SPL_SCALEDIFF32(filter_coefficients[1], diff, out_data[k-1]);
    }
    ...
    diff = VOIP_SPL_SUB_SAT_W32(in_data[0], filter_state[5]); // = (y_2[0] - y[-1])
    // y[0] =  y_2[-1] + a_3 * (y_2[0] - y[-1])
    out_data[0] = VOIP_SPL_SCALEDIFF32(filter_coefficients[2], diff, filter_state[4]);
    for (k = 1; k < data_length; k++)
    {
        diff = VOIP_SPL_SUB_SAT_W32(in_data[k], out_data[k - 1]); // = (y_2[n] - y[n-1])
        // y[n] =  y_2[n-1] + a_3 * (y_2[n] - y[n-1])
        out_data[k] = VOIP_SPL_SCALEDIFF32(filter_coefficients[2], diff, in_data[k-1]);
    }
    filter_state[4] = in_data[data_length - 1]; // y_2[N-1], becomes y_2[-1] next time
    filter_state[5] = out_data[data_length - 1]; // y[N-1], becomes y[-1] next time
}

void VoipSpl_AnalysisQMF(const int16_t* in_data, int in_data_length,
                           ...
                           int32_t* filter_state1, int32_t* filter_state2)
{
    ...
    {
        half_in2[i] = VOIP_SPL_LSHIFT_W32((int32_t)in_data[k], 10);
        half_in1[i] = VOIP_SPL_LSHIFT_W32((int32_t)in_data[k + 1], 10);
    }

    // All pass filter even and odd samples, independently.
    VoipSpl_AllPassQMF(half_in1, band_length, filter1,
                         VoipSpl_kAllPassFilter1, filter_state1);
    VoipSpl_AllPassQMF(half_in2, band_length, filter2,
                         VoipSpl_kAllPassFilter2, filter_state2);
    ...
    for (i = 0; i < band_length; i++)
    {
        ...
        tmp = VOIP_SPL_RSHIFT_W32(tmp, 11);
        low_band[i] = VoipSpl_SatW32ToW16(tmp);

        tmp = filter1[i] - filter2[i] + 1024;
        tmp = VOIP_SPL_RSHIFT_W32(tmp, 11);
        high_band[i] = VoipSpl_SatW32ToW16(tmp);
    }
}

void VoipSpl_SynthesisQMF(const int16_t* low_band, const int16_t* high_band,
                            ...
                            int32_t* filter_state1, int32_t* filter_state2)
{
    ...
    for (i = 0; i < band_length; i++)
    {
        ...
        half_in1[i] = VOIP_SPL_LSHIFT_W32(tmp, 10);
        tmp = (int32_t)low_band[i] - (int32_t)high_band[i];
        half_in2[i] = VOIP_SPL_LSHIFT_W32(tmp, 10);
    }

    // all-pass filter the sum and difference channels
    VoipSpl_AllPassQMF(half_in1, band_length, filter1,
                         VoipSpl_kAllPassFilter2, filter_state1);
    VoipSpl_AllPassQMF(half_in2, band_length, filter2,
                         VoipSpl_kAllPassFilter1, filter_state2);
    ...
    {
        tmp = VOIP_SPL_RSHIFT_W32(filter2[i] + 512, 10);
        out_data[k++] = VoipSpl_SatW32ToW16(tmp);

        tmp = VOIP_SPL_RSHIFT_W32(filter1[i] + 512, 10);
        out_data[k++] = VoipSpl_SatW32ToW16(tmp);
    }
