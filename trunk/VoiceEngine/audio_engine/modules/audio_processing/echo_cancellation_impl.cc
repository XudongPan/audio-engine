...
int EchoCancellationImpl::ProcessCaptureAudio(AudioBuffer* audio) {
  ...
  for (int i = 0; i < audio->num_channels(); i++) {
    for (int j = 0; j < apm_->num_reverse_channels(); j++) {
      ...
      int status = 0;
      err = VoipAec_get_echo_status(my_handle, &status);
      if (err != apm_->kNoError) {
...
int EchoCancellationImpl::GetMetrics(Metrics* metrics) {
  ...
  int err = VoipAec_GetMetrics(my_handle, &my_metrics);
  if (err != apm_->kNoError) {
    return GetHandleError(my_handle);
  }
...
int EchoCancellationImpl::GetDelayMetrics(int* median, int* std) {
  ...
  Handle* my_handle = static_cast<Handle*>(handle(0));
  if (VoipAec_GetDelayMetrics(my_handle, median, std) !=
      apm_->kNoError) {
    return GetHandleError(my_handle);
  }
...
struct AecCore* EchoCancellationImpl::aec_core() const {
  ...
  Handle* my_handle = static_cast<Handle*>(handle(0));
  return VoipAec_aec_core(my_handle);
}
...
void* EchoCancellationImpl::CreateHandle() const {
  Handle* handle = NULL;
  if (VoipAec_Create(&handle) != apm_->kNoError) {
    handle = NULL;
  } else {
    assert(handle != NULL);
  }
...
int EchoCancellationImpl::DestroyHandle(void* handle) const {
  assert(handle != NULL);
  return VoipAec_Free(static_cast<Handle*>(handle));
}

int EchoCancellationImpl::InitializeHandle(void* handle) const {
  assert(handle != NULL);
  return VoipAec_Init(static_cast<Handle*>(handle),
                       apm_->sample_rate_hz(),
...
int EchoCancellationImpl::ConfigureHandle(void* handle) const {
  ...
  VoipAec_enable_delay_correction(VoipAec_aec_core(
      static_cast<Handle*>(handle)), delay_correction_enabled_ ? 1 : 0);
  return VoipAec_set_config(static_cast<Handle*>(handle), config);
}
...
int EchoCancellationImpl::GetHandleError(void* handle) const {
  assert(handle != NULL);
  return MapError(VoipAec_get_error_code(static_cast<Handle*>(handle)));
}
}  // namespace VoIP
