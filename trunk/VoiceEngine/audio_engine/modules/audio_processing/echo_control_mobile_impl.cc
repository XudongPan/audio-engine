...
size_t EchoControlMobile::echo_path_size_bytes() {
    return VoipAecm_echo_path_size_bytes();
}
...
int EchoControlMobileImpl::GetEchoPath(void* echo_path,
  ...
  if (VoipAecm_GetEchoPath(my_handle, echo_path, size_bytes) != 0) {
...
void* EchoControlMobileImpl::CreateHandle() const {
  Handle* handle = NULL;
  if (VoipAecm_Create(&handle) != apm_->kNoError) {
    handle = NULL;
  } else {
    assert(handle != NULL);
  }
...
int EchoControlMobileImpl::DestroyHandle(void* handle) const {
  return VoipAecm_Free(static_cast<Handle*>(handle));
}

int EchoControlMobileImpl::InitializeHandle(void* handle) const {
  assert(handle != NULL);
  Handle* my_handle = static_cast<Handle*>(handle);
  if (VoipAecm_Init(my_handle, apm_->sample_rate_hz()) != 0) {
    return GetHandleError(my_handle);
  }
  if (external_echo_path_ != NULL) {
    if (VoipAecm_InitEchoPath(my_handle,
                                external_echo_path_,
...
int EchoControlMobileImpl::ConfigureHandle(void* handle) const {
  ...
  return VoipAecm_set_config(static_cast<Handle*>(handle), config);
}
...
int EchoControlMobileImpl::GetHandleError(void* handle) const {
  assert(handle != NULL);
  return MapError(VoipAecm_get_error_code(static_cast<Handle*>(handle)));
}
}  // namespace VoIP
