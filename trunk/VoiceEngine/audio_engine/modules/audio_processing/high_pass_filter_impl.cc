...
int InitializeFilter(FilterState* hpf, int sample_rate_hz) {
  ...
  VoipSpl_MemSetW16(hpf->x, 0, 2);
  VoipSpl_MemSetW16(hpf->y, 0, 4);
...
int Filter(FilterState* hpf, int16_t* data, int length) {
  ...
  for (int i = 0; i < length; i++) {
    ...
    tmp_int32 =
        VOIP_SPL_MUL_16_16(y[1], ba[3]); // -a[1] * y[i-1] (low part)
    tmp_int32 +=
        VOIP_SPL_MUL_16_16(y[3], ba[4]); // -a[2] * y[i-2] (low part)
    tmp_int32 = (tmp_int32 >> 15);
    tmp_int32 +=
        VOIP_SPL_MUL_16_16(y[0], ba[3]); // -a[1] * y[i-1] (high part)
    tmp_int32 +=
        VOIP_SPL_MUL_16_16(y[2], ba[4]); // -a[2] * y[i-2] (high part)
    tmp_int32 = (tmp_int32 << 1);

    tmp_int32 += VOIP_SPL_MUL_16_16(data[i], ba[0]); // b[0]*x[0]
    tmp_int32 += VOIP_SPL_MUL_16_16(x[0], ba[1]);    // b[1]*x[i-1]
    tmp_int32 += VOIP_SPL_MUL_16_16(x[1], ba[2]);    // b[2]*x[i-2]
    ...
    y[1] = static_cast<int16_t>((tmp_int32 -
        VOIP_SPL_LSHIFT_W32(static_cast<int32_t>(y[0]), 13)) << 2);

    // Rounding in Q12, i.e. add 2^11
    tmp_int32 += 2048;

    // Saturate (to 2^27) so that the HP filtered signal does not overflow
    tmp_int32 = VOIP_SPL_SAT(static_cast<int32_t>(134217727),
                               tmp_int32,
                               static_cast<int32_t>(-134217728));

    // Convert back to Q0 and use rounding
    data[i] = (int16_t)VOIP_SPL_RSHIFT_W32(tmp_int32, 12);
