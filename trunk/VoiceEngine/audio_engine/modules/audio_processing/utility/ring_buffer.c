...
static size_t GetBufferReadRegions(RingBuffer* buf,
  ...
  const size_t readable_elements = Voip_available_read(buf);
...
RingBuffer* Voip_CreateBuffer(size_t element_count, size_t element_size) {
...
int Voip_InitBuffer(RingBuffer* self) {
  if (!self) {
    return -1;
  }
...
void Voip_FreeBuffer(void* handle) {
...
size_t Voip_ReadBuffer(RingBuffer* self,
                         void** data_ptr,
                         void* data,
                         size_t element_count) {
...

  {
    ...
    Voip_MoveReadPtr(self, (int) read_count);

    return read_count;
  }
}

size_t Voip_WriteBuffer(RingBuffer* self,
                          const void* data,
                          size_t element_count) {
  if (!self) {
    return 0;
  ...
  {
    const size_t free_elements = Voip_available_write(self);
...
int Voip_MoveReadPtr(RingBuffer* self, int element_count) {
  if (!self) {
    return 0;
  }

  {
    ...
    // instead of "size_t".
    const int free_elements = (int) Voip_available_write(self);
    const int readable_elements = (int) Voip_available_read(self);
...
size_t Voip_available_read(const RingBuffer* self) {
  if (!self) {
    return 0;
  }
...
size_t Voip_available_write(const RingBuffer* self) {
  if (!self) {
    return 0;
  }

  return self->element_count - Voip_available_read(self);
}
