...
extern const float VoipAec_weightCurve[65];
extern const float VoipAec_overDriveCurve[65];
...
static void OverdriveAndSuppressSSE2(AecCore* aec,
  ...
  for (i = 0; i + 3 < PART_LEN1; i += 4) {
    ...
    const __m128 vec_weightCurve = _mm_loadu_ps(&VoipAec_weightCurve[i]);
...

    {
      const __m128 vec_overDriveCurve =
          _mm_loadu_ps(&VoipAec_overDriveCurve[i]);
...
void VoipAec_InitAec_SSE2(void) {
  VoipRtcAec_FilterFar = FilterFarSSE2;
  VoipAec_ScaleErrorSignal = ScaleErrorSignalSSE2;
  VoipAec_FilterAdaptation = FilterAdaptationSSE2;
  VoipAec_OverdriveAndSuppress = OverdriveAndSuppressSSE2;
}
