#include "audio_engine/modules/audio_processing/aecm/aecm_core.h"
...
#include "audio_engine/modules/audio_processing/aecm/include/echo_control_mobile.h"
...
const int16_t VoipAecm_kCosTable[] = {
...
const int16_t VoipAecm_kSinTable[] = {
...
CalcLinearEnergies VoipAecm_CalcLinearEnergies;
StoreAdaptiveChannel VoipAecm_StoreAdaptiveChannel;
ResetAdaptiveChannel VoipAecm_ResetAdaptiveChannel;

int VoipAecm_CreateCore(AecmCore_t **aecmInst)
{
    ...
    *aecmInst = aecm;
    ...
    aecm->farFrameBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN,
                                            sizeof(int16_t));
    if (!aecm->farFrameBuf)
    {
        VoipAecm_FreeCore(aecm);
        aecm = NULL;
        return -1;
    }

    aecm->nearNoisyFrameBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN,
                                                  sizeof(int16_t));
    ...
    aecm->nearCleanFrameBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN,
                                                  sizeof(int16_t));
    ...
    aecm->outFrameBuf = Voip_CreateBuffer(FRAME_LEN + PART_LEN,
                                            sizeof(int16_t));
    if (!aecm->outFrameBuf)
    {
        VoipAecm_FreeCore(aecm);
    ...
    aecm->delay_estimator_farend = Voip_CreateDelayEstimatorFarend(PART_LEN1,
                                                                     MAX_DELAY);
    ...
    aecm->delay_estimator =
        Voip_CreateDelayEstimator(aecm->delay_estimator_farend, 0);
    ...
    Voip_enable_robust_validation(aecm->delay_estimator, 0);

    aecm->real_fft = VoipSpl_CreateRealFFT(PART_LEN_SHIFT);
...
void VoipAecm_InitEchoPathCore(AecmCore_t* aecm, const int16_t* echo_path)
{
    int i = 0;
    ...
    {
        aecm->channelAdapt32[i] = VOIP_SPL_LSHIFT_W32(
    ...
    aecm->mseThreshold = VOIP_SPL_WORD32_MAX;
                                ...
                                uint32_t* echo_energy_stored)
{
    ...
    for (i = 0; i < PART_LEN1; i++)
    {
        echo_est[i] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i],
                                           far_spectrum[i]);
        (*far_energy) += (uint32_t)(far_spectrum[i]);
        (*echo_energy_adapt) += VOIP_SPL_UMUL_16_16(aecm->channelAdapt16[i],
                                          far_spectrum[i]);
                                  ...
                                  int32_t* echo_est)
{
    ...
    for (i = 0; i < PART_LEN; i += 4)
    {
        echo_est[i] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i],
                                           far_spectrum[i]);
        echo_est[i + 1] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i + 1],
                                           far_spectrum[i + 1]);
        echo_est[i + 2] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i + 2],
                                           far_spectrum[i + 2]);
        echo_est[i + 3] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i + 3],
                                           far_spectrum[i + 3]);
    }
    echo_est[i] = VOIP_SPL_MUL_16_U16(aecm->channelStored[i],
                                       far_spectrum[i]);
...
static void ResetAdaptiveChannelC(AecmCore_t* aecm)
{
    ...
    for (i = 0; i < PART_LEN; i += 4)
    {
        ...
        aecm->channelAdapt32[i + 1] = VOIP_SPL_LSHIFT_W32(
                (int32_t)aecm->channelStored[i + 1], 16);
        aecm->channelAdapt32[i + 2] = VOIP_SPL_LSHIFT_W32(
                (int32_t)aecm->channelStored[i + 2], 16);
        aecm->channelAdapt32[i + 3] = VOIP_SPL_LSHIFT_W32(
                (int32_t)aecm->channelStored[i + 3], 16);
    }
    aecm->channelAdapt32[i] = VOIP_SPL_LSHIFT_W32((int32_t)aecm->channelStored[i], 16);
}

// Initialize function pointers for ARM Neon platform.
#if (defined VOIP_DETECT_ARM_NEON || defined VOIP_ARCH_ARM_NEON)
static void VoipAecm_InitNeon(void)
{
  VoipAecm_StoreAdaptiveChannel = VoipAecm_StoreAdaptiveChannelNeon;
  VoipAecm_ResetAdaptiveChannel = VoipAecm_ResetAdaptiveChannelNeon;
  VoipAecm_CalcLinearEnergies = VoipAecm_CalcLinearEnergiesNeon;
}
#endif

// Initialize function pointers for MIPS platform.
#if defined(MIPS32_LE)
static void VoipAecm_InitMips(void)
{
#if defined(MIPS_DSP_R1_LE)
  VoipAecm_StoreAdaptiveChannel = VoipAecm_StoreAdaptiveChannel_mips;
  VoipAecm_ResetAdaptiveChannel = VoipAecm_ResetAdaptiveChannel_mips;
#endif
  VoipAecm_CalcLinearEnergies = VoipAecm_CalcLinearEnergies_mips;
}
#endif

// VoIPAecm_InitCore(...)
//
// This function initializes the AECM instant created with VoIPAecm_CreateCore(...)
// Input:
...
int VoipAecm_InitCore(AecmCore_t * const aecm, int samplingFreq)
{
    ...
    Voip_InitBuffer(aecm->farFrameBuf);
    Voip_InitBuffer(aecm->nearNoisyFrameBuf);
    Voip_InitBuffer(aecm->nearCleanFrameBuf);
    Voip_InitBuffer(aecm->outFrameBuf);
    ...
    if (Voip_InitDelayEstimatorFarend(aecm->delay_estimator_farend) != 0) {
      return -1;
    }
    if (Voip_InitDelayEstimator(aecm->delay_estimator) != 0) {
      return -1;
    }
    ...
    {
        VoipAecm_InitEchoPathCore(aecm, kChannelStored8kHz);
    }
    else
    {
        VoipAecm_InitEchoPathCore(aecm, kChannelStored16kHz);
    }
    ...
    aecm->farEnergyMin = VOIP_SPL_WORD16_MAX;
    aecm->farEnergyMax = VOIP_SPL_WORD16_MIN;
    ...
    VoipAecm_CalcLinearEnergies = CalcLinearEnergiesC;
    VoipAecm_StoreAdaptiveChannel = StoreAdaptiveChannelC;
    VoipAecm_ResetAdaptiveChannel = ResetAdaptiveChannelC;

#ifdef VOIP_DETECT_ARM_NEON
    uint64_t features = Voip_GetCPUFeaturesARM();
...
int VoipAecm_FreeCore(AecmCore_t *aecm)
{
    ...
    Voip_FreeBuffer(aecm->farFrameBuf);
    Voip_FreeBuffer(aecm->nearNoisyFrameBuf);
    Voip_FreeBuffer(aecm->nearCleanFrameBuf);
    Voip_FreeBuffer(aecm->outFrameBuf);

    Voip_FreeDelayEstimator(aecm->delay_estimator);
    Voip_FreeDelayEstimatorFarend(aecm->delay_estimator_farend);
    VoipSpl_FreeRealFFT(aecm->real_fft);
...
int VoipAecm_ProcessFrame(AecmCore_t * aecm,
                            ...
                            int16_t * out)
{
    ...
    VoipAecm_BufferFarFrame(aecm, farend, FRAME_LEN);
    VoipAecm_FetchFarFrame(aecm, farFrame, FRAME_LEN, aecm->knownDelay);

    // Buffer the synchronized far and near frames,
    // to pass the smaller blocks individually.
    Voip_WriteBuffer(aecm->farFrameBuf, farFrame, FRAME_LEN);
    Voip_WriteBuffer(aecm->nearNoisyFrameBuf, nearendNoisy, FRAME_LEN);
    if (nearendClean != NULL)
    {
        Voip_WriteBuffer(aecm->nearCleanFrameBuf, nearendClean, FRAME_LEN);
    }

    // Process as many blocks as possible.
    while (Voip_available_read(aecm->farFrameBuf) >= PART_LEN)
    {
        ...
        Voip_ReadBuffer(aecm->farFrameBuf, (void**) &far_block_ptr, far_block,
                          PART_LEN);
        Voip_ReadBuffer(aecm->nearNoisyFrameBuf,
        ...
        if (nearendClean != NULL)
        {
            ...
            Voip_ReadBuffer(aecm->nearCleanFrameBuf,
                              (void**) &near_clean_block_ptr,
                              near_clean_block,
                              PART_LEN);
//             if (VoIPAecm_ProcessBlock(aecm,
//                                         far_block_ptr,
...
//             }
        ...
        Voip_WriteBuffer(aecm->outFrameBuf, outBlock, PART_LEN);
    }

    // Stuff the out buffer if we have less than a frame to output.
    // This should only happen for the first frame.
    size = (int) Voip_available_read(aecm->outFrameBuf);
    if (size < FRAME_LEN)
    {
        Voip_MoveReadPtr(aecm->outFrameBuf, size - FRAME_LEN);
    }

    // Obtain an output frame.
    Voip_ReadBuffer(aecm->outFrameBuf, (void**) &out_ptr, out, FRAME_LEN);
    if (out_ptr != out) {
...
//
int16_t VoipAecm_AsymFilt(const int16_t filtOld, const int16_t inVal,
                            const int16_t stepSizePos,
                            const int16_t stepSizeNeg)
{
    int16_t retVal;

    if ((filtOld == VOIP_SPL_WORD16_MAX) | (filtOld == VOIP_SPL_WORD16_MIN))
    {
        return inVal;
    }
    retVal = filtOld;
    if (filtOld > inVal)
    {
        retVal -= VOIP_SPL_RSHIFT_W16(filtOld - inVal, stepSizeNeg);
    } else
    {
        retVal += VOIP_SPL_RSHIFT_W16(inVal - filtOld, stepSizePos);
    }
                             ...
                             int32_t * echoEst)
{
    ...
    int16_t kLogLowValue = VOIP_SPL_LSHIFT_W16(PART_LEN_SHIFT, 7);
    ...
    if (nearEner)
    {
        zeros = VoipSpl_NormU32(nearEner);
        frac = (int16_t)VOIP_SPL_RSHIFT_U32(
                              (VOIP_SPL_LSHIFT_U32(nearEner, zeros) & 0x7FFFFFFF),
                              23);
        // log2 in Q8
        tmp16 += VOIP_SPL_LSHIFT_W16((31 - zeros), 8) + frac;
        tmp16 -= VOIP_SPL_LSHIFT_W16(aecm->dfaNoisyQDomain, 8);
    }
    aecm->nearLogEnergy[0] = tmp16;
    // END: Get log of near end energy

    VoipAecm_CalcLinearEnergies(aecm, far_spectrum, echoEst, &tmpFar, &tmpAdapt, &tmpStored);
    ...
    if (tmpFar)
    {
        zeros = VoipSpl_NormU32(tmpFar);
        frac = (int16_t)VOIP_SPL_RSHIFT_U32((VOIP_SPL_LSHIFT_U32(tmpFar, zeros)
                        & 0x7FFFFFFF), 23);
        // log2 in Q8
        tmp16 += VOIP_SPL_LSHIFT_W16((31 - zeros), 8) + frac;
        tmp16 -= VOIP_SPL_LSHIFT_W16(far_q, 8);
    }
    ...
    if (tmpAdapt)
    {
        zeros = VoipSpl_NormU32(tmpAdapt);
        frac = (int16_t)VOIP_SPL_RSHIFT_U32((VOIP_SPL_LSHIFT_U32(tmpAdapt, zeros)
                        & 0x7FFFFFFF), 23);
        //log2 in Q8
        tmp16 += VOIP_SPL_LSHIFT_W16((31 - zeros), 8) + frac;
        tmp16 -= VOIP_SPL_LSHIFT_W16(RESOLUTION_CHANNEL16 + far_q, 8);
    }
    ...
    if (tmpStored)
    {
        zeros = VoipSpl_NormU32(tmpStored);
        frac = (int16_t)VOIP_SPL_RSHIFT_U32((VOIP_SPL_LSHIFT_U32(tmpStored, zeros)
                        & 0x7FFFFFFF), 23);
        //log2 in Q8
        tmp16 += VOIP_SPL_LSHIFT_W16((31 - zeros), 8) + frac;
        tmp16 -= VOIP_SPL_LSHIFT_W16(RESOLUTION_CHANNEL16 + far_q, 8);
    ...
    if (aecm->farLogEnergy > FAR_ENERGY_MIN)
    {
        ...
        aecm->farEnergyMin = VoipAecm_AsymFilt(aecm->farEnergyMin, aecm->farLogEnergy,
                                                 increase_min_shifts, decrease_min_shifts);
        aecm->farEnergyMax = VoipAecm_AsymFilt(aecm->farEnergyMax, aecm->farLogEnergy,
        ...
        if (tmp16 > 0)
        {
            tmp16 = (int16_t)VOIP_SPL_MUL_16_16_RSFT(tmp16, FAR_ENERGY_VAD_REGION, 9);
        } else
        {
            tmp16 = 0;
        }
        ...
        } else
        {
            ...
            {
                aecm->farEnergyVAD += VOIP_SPL_RSHIFT_W16(aecm->farLogEnergy +
                                                            tmp16 -
                                                            aecm->farEnergyVAD,
                                                            6);
                aecm->vadUpdateCount = 0;
...
int16_t VoipAecm_CalcStepSize(AecmCore_t * const aecm)
{
    ...
    } else if (aecm->startupState > 0)
    {
        ...
        } else
        {
            ...
            tmp32 = VOIP_SPL_MUL_16_16(tmp16, MU_DIFF);
            tmp32 = VoipSpl_DivW32W16(tmp32, aecm->farEnergyMaxMin);
                              ...
                              int32_t * echoEst)
{
    ...
    if (mu)
    {
        for (i = 0; i < PART_LEN1; i++)
        {
            ...
            // multiplication
            zerosCh = VoipSpl_NormU32(aecm->channelAdapt32[i]);
            zerosFar = VoipSpl_NormU32((uint32_t)far_spectrum[i]);
            if (zerosCh + zerosFar > 31)
            {
                // Multiplication is safe
                tmpU32no1 = VOIP_SPL_UMUL_32_16(aecm->channelAdapt32[i],
                        far_spectrum[i]);
                shiftChFar = 0;
            } else
            {
                ...
                tmpU32no1 = VOIP_SPL_UMUL_32_16(
                    VOIP_SPL_RSHIFT_W32(aecm->channelAdapt32[i], shiftChFar),
                    far_spectrum[i]);
            }
            // Determine Q-domain of numerator
            zerosNum = VoipSpl_NormU32(tmpU32no1);
            if (dfa[i])
            {
                zerosDfa = VoipSpl_NormU32((uint32_t)dfa[i]);
            } else
            {
                zerosDfa = 32;
            }
            ...
            tmpU32no1 = VOIP_SPL_SHIFT_W32(tmpU32no1, xfaQ);
            tmpU32no2 = VOIP_SPL_SHIFT_W32((uint32_t)dfa[i], dfaQ);
            tmp32no1 = (int32_t)tmpU32no2 - (int32_t)tmpU32no1;
            zerosNum = VoipSpl_NormW32(tmp32no1);
            if ((tmp32no1) && (far_spectrum[i] > (CHANNEL_VAD << far_q)))
            {
                ...
                if (zerosNum + zerosFar > 31)
                {
                    if (tmp32no1 > 0)
                    {
                        tmp32no2 = (int32_t)VOIP_SPL_UMUL_32_16(tmp32no1,
                                                                        far_spectrum[i]);
                    } else
                    {
                        tmp32no2 = -(int32_t)VOIP_SPL_UMUL_32_16(-tmp32no1,
                ...
                } else
                {
                    ...
                    if (tmp32no1 > 0)
                    {
                        tmp32no2 = (int32_t)VOIP_SPL_UMUL_32_16(
                                VOIP_SPL_RSHIFT_W32(tmp32no1, shiftNum),
                                far_spectrum[i]);
                    } else
                    {
                        tmp32no2 = -(int32_t)VOIP_SPL_UMUL_32_16(
                ...
                tmp32no2 = VoipSpl_DivW32W16(tmp32no2, i + 1);
                // Make sure we are in the right Q-domain
                shift2ResChan = shiftNum + shiftChFar - xfaQ - mu - ((30 - zerosFar) << 1);
                if (VoipSpl_NormW32(tmp32no2) < shift2ResChan)
                {
                    tmp32no2 = VOIP_SPL_WORD32_MAX;
                } else
                {
                    tmp32no2 = VOIP_SPL_SHIFT_W32(tmp32no2, shift2ResChan);
                }
                aecm->channelAdapt32[i] = VOIP_SPL_ADD_SAT_W32(aecm->channelAdapt32[i],
                        tmp32no2);
                ...
                aecm->channelAdapt16[i]
                        = (int16_t)VOIP_SPL_RSHIFT_W32(aecm->channelAdapt32[i], 16);
            }
        }
    }
    ...
    if ((aecm->startupState == 0) & (aecm->currentVADValue))
    {
        ...
        VoipAecm_StoreAdaptiveChannel(aecm, far_spectrum, echoEst);
    } else
    {
        ...
        if (aecm->mseChannelCount >= (MIN_MSE_COUNT + 10))
        {
            ...
            for (i = 0; i < MIN_MSE_COUNT; i++)
            {
                ...
                tmp32no2 = VOIP_SPL_ABS_W32(tmp32no1);
                mseStored += tmp32no2;
                            ...
                            * aecm->mseAdaptOld)))
            {
                // The stored channel has a significantly lower MSE than the adaptive one for
                // two consecutive calculations. Reset the adaptive channel.
                VoipAecm_ResetAdaptiveChannel(aecm);
                    ...
                    < aecm->mseThreshold) & (aecm->mseAdaptOld < aecm->mseThreshold))
            {
                ...
                // Update threshold
                if (aecm->mseThreshold == VOIP_SPL_WORD32_MAX)
                {
                    aecm->mseThreshold = (mseAdapt + aecm->mseAdaptOld);
                } else
                {
                    aecm->mseThreshold += VOIP_SPL_MUL_16_16_RSFT(mseAdapt
                            - VOIP_SPL_MUL_16_16_RSFT(aecm->mseThreshold, 5, 3), 205, 8);
                }
...
int16_t VoipAecm_CalcSuppressionGain(AecmCore_t * const aecm)
{
    ...
    } else
    {
        ...
        if (dE < ENERGY_DEV_TOL)
        {
            ...
            {
                tmp32no1 = VOIP_SPL_MUL_16_16(aecm->supGainErrParamDiffAB, dE);
                tmp32no1 += (SUPGAIN_EPC_DT >> 1);
                tmp16no1 = (int16_t)VoipSpl_DivW32W16(tmp32no1, SUPGAIN_EPC_DT);
                supGain = aecm->supGainErrParamA - tmp16no1;
            } else
            {
                tmp32no1 = VOIP_SPL_MUL_16_16(aecm->supGainErrParamDiffBD,
                                                (ENERGY_DEV_TOL - dE));
                tmp32no1 += ((ENERGY_DEV_TOL - SUPGAIN_EPC_DT) >> 1);
                tmp16no1 = (int16_t)VoipSpl_DivW32W16(tmp32no1, (ENERGY_DEV_TOL
                        - SUPGAIN_EPC_DT));
...
void VoipAecm_FetchFarFrame(AecmCore_t * const aecm, int16_t * const farend,
