#include "audio_engine/modules/audio_processing/audio_processing_impl.h"
...
#include "audio_engine/modules/audio_processing/echo_cancellation_impl.h"
#include "audio_engine/modules/audio_processing/echo_control_mobile_impl.h"
#include "audio_engine/modules/audio_processing/gain_control_impl.h"
#include "audio_engine/modules/audio_processing/high_pass_filter_impl.h"
#include "audio_engine/modules/audio_processing/level_estimator_impl.h"
...
#include "audio_engine/system_wrappers/interface/compile_assert.h"
...
#ifdef VOIP_AUDIOPROC_DEBUG_DUMP
// Files generated at build-time by the protobuf compiler.
#ifdef VOIP_ANDROID_PLATFORM_BUILD
#include "external/VoIP/VoIP/modules/audio_processing/debug.pb.h"
#else
#include "audio_engine/audio_processing/debug.pb.h"
#endif
#endif  // VOIP_AUDIOPROC_DEBUG_DUMP
...
// Throughout VoIP, it's assumed that success is represented by zero.
...
int AudioProcessingImpl::ProcessStreamLocked() {
  ...
  if (analysis_needed(data_processed)) {
    for (int i = 0; i < num_output_channels_; i++) {
      ...
      VoipSpl_AnalysisQMF(capture_audio_->data(i),
  ...
  if (synthesis_needed(data_processed)) {
    for (int i = 0; i < num_output_channels_; i++) {
      ...
      VoipSpl_SynthesisQMF(capture_audio_->low_pass_split_data(i),
                             capture_audio_->high_pass_split_data(i),
...
int AudioProcessingImpl::AnalyzeReverseStreamLocked() {
  if (sample_rate_hz_ == kSampleRate32kHz) {
    for (int i = 0; i < num_reverse_channels_; i++) {
      ...
      VoipSpl_AnalysisQMF(render_audio_->data(i),
...
#if defined(VOIP_ARCH_BIG_ENDIAN)
