...
int32_t VoipVad_GaussianProbability(int16_t input,
  ...
  inv_std = (int16_t) VoipSpl_DivW32W16(tmp32, std);

  // Calculate |inv_std2| = 1 / s^2, in Q14.
  tmp16 = (inv_std >> 2);  // Q10 -> Q8.
  // Q-domain: (Q8 * Q8) >> 2 = Q14.
  inv_std2 = (int16_t) VOIP_SPL_MUL_16_16_RSFT(tmp16, tmp16, 2);
  // TODO(bjornv): Investigate if changing to
  // |inv_std2| = (int16_t) VOIP_SPL_MUL_16_16_RSFT(|inv_std|, |inv_std|, 6);
  ...
  *delta = (int16_t) VOIP_SPL_MUL_16_16_RSFT(inv_std2, tmp16, 10);

  // Calculate the exponent |tmp32| = (x - m)^2 / (2 * s^2), in Q10. Replacing
  // division by two with one shift.
  // Q-domain: (Q11 * Q7) >> 8 = Q10.
  tmp32 = VOIP_SPL_MUL_16_16_RSFT(*delta, tmp16, 9);
  ...
  if (tmp32 < kCompVar) {
    ...
    tmp16 = (int16_t) VOIP_SPL_MUL_16_16_RSFT(kLog2Exp, (int16_t) tmp32, 12);
    tmp16 = -tmp16;
  ...
  return VOIP_SPL_MUL_16_16(inv_std, exp_value);
}
