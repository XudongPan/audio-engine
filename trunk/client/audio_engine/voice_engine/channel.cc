...
#include "audio_engine/modules/audio_package/interface/receive_statistics.h"
#include "audio_engine/modules/audio_package/interface/audio_pkg_payload_registry.h"
#include "audio_engine/modules/audio_package/interface/audio_pkg_receiver.h"
#include "audio_engine/modules/audio_package/source/audio_pkg_receiver_strategy.h"
...
#include "audio_engine/modules/utility/interface/audio_package_dump.h"
...
#include "audio_engine/voice_engine/include/voe_base.h"
#include "audio_engine/voice_engine/include/voe_external_media.h"
#include "audio_engine/voice_engine/include/voe_rtp_rtcp.h"
                  ...
                  const RTPFragmentationHeader* fragmentation)
{
                                        ...
                                        fragmentation) == -1)
    {
        _engineStatisticsPtr->SetLastError(
            VE_AUDIO_PACKET_MODULE_ERROR, kTraceWarning,
...
int
Channel::SendCtrlPacket(int channel, const void *data, int len)
{
    channel = VoEChannelId(channel);
    assert(channel == _channelId);

    WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId,_channelId),
                 "Channel::SendCtrlPacket(channel=%d, len=%d)", channel, len);
    ...
    if (_transportPtr == NULL)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice,
                     VoEId(_instanceId,_channelId),
                     "Channel::SendCtrlPacket() failed to send RTCP packet"
    ...
    int n = _transportPtr->SendCtrlPacket(channel,
                                          bufferToSendPtr,
                                          bufferLength);
    if (n < 0) {
      std::string transport_name =
      ...
      WEBRTC_TRACE(kTraceInfo, kTraceVoice,
                   VoEId(_instanceId,_channelId),
                   "Channel::SendCtrlPacket() transmission using %s failed",
                   transport_name.c_str());
      return -1;
    }
    return n;
    ...
    uint32_t rate)
{
    ...
    strncpy(receiveCodec.plname, payloadName, PAYLOAD_NAME_SIZE - 1);

    audio_coding_->Codec(payloadName, &dummyCodec, frequency, channels);
    receiveCodec.packsize = dummyCodec.packsize;
...
bool Channel::OnRecoveredPacket(const uint8_t* rtp_packet,
  ...
  if (!rtp_header_parser_->Parse(rtp_packet, rtp_packet_length, &header)) {
    WEBRTC_TRACE(kTraceDebug, VoIP::kTraceVoice, _channelId,
...
Channel::Init()
{
    ...
    {
        _engineStatisticsPtr->SetLastError(
            VE_AUDIO_PACKET_MODULE_ERROR, kTraceError,
...
Channel::StopSend()
{
    ...
    // https://code.google.com/p/VoIP/issues/detail?id=2111 .
...
int32_t
Channel::SetNetEQPlayoutMode(AudioCacheModes mode)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
...
int32_t
Channel::GetNetEQPlayoutMode(AudioCacheModes& mode)
{
...
int32_t
Channel::RegisterVoiceEngineObserver(AudioEngineObserver& observer)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId,_channelId),
...
Channel::SetSendCodec(const CodecInst& codec)
{
    ...
    if (_rtpRtcpModule->SetAudioPacketSize(codec.packsize) != 0)
    {
        WEBRTC_TRACE(kTraceError, kTraceVoice, VoEId(_instanceId,_channelId),
...
int32_t Channel::ReceivedRTPPacket(const int8_t* data, int32_t length,
  ...
  if (!rtp_header_parser_->Parse(received_packet, length, &header)) {
    WEBRTC_TRACE(VoIP::kTraceDebug, VoIP::kTraceVoice, _channelId,
...
bool Channel::IsPacketInOrder(const AudioPacketHeader& header) const {
  StreamStatistician* statistician =
...
bool Channel::IsPacketRetransmitted(const AudioPacketHeader& header,
                                    bool in_order) const {
...
int Channel::GetRemoteRTCPSenderInfo(SenderInfo* sender_info) {
  ...
  if (_rtpRtcpModule->RemoteRTCPStat(&rtcp_sender_info) != 0) {
    _engineStatisticsPtr->SetLastError(VE_AUDIO_PACKET_MODULE_ERROR, kTraceError,
...
Channel::StartRTPDump(const char fileNameUTF8[1024],
                      AudioPacketDirections direction)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
                 "Channel::StartRTPDump()");
    if ((direction != kIncoming) && (direction != kOutgoing))
    {
        _engineStatisticsPtr->SetLastError(
            VE_INVALID_ARGUMENT, kTraceError,
    ...
    RtpDump* rtpDumpPtr = (direction == kIncoming) ?
...
int
Channel::StopRTPDump(AudioPacketDirections direction)
{
    WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, _channelId),
                 "Channel::StopRTPDump()");
    if ((direction != kIncoming) && (direction != kOutgoing))
...
bool
Channel::RTPDumpIsActive(AudioPacketDirections direction)
{
    if ((direction != kIncoming) &&
        (direction != kOutgoing))
    {
        _engineStatisticsPtr->SetLastError(
            VE_INVALID_ARGUMENT, kTraceError,
...
Channel::SendPacketRaw(const void *data, int len, bool RTCP)
{
    ...
    {
        return _transportPtr->SendCtrlPacket(_channelId, data, len);
    }
}
...
void Channel::OnPlayTelephoneEvent( const int32_t id, const uint8_t event, const uint16_t lengthMs, const uint8_t volume )
{
