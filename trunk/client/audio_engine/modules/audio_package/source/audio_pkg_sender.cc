...
#include "audio_engine/modules/audio_package/source/audio_pkg_sender_audio.h"
#include "audio_engine/modules/audio_package/source/audio_pkg_sender_video.h"
...
int32_t RTPSender::RegisterPayload(
    const char payload_name[PAYLOAD_NAME_SIZE],
...
int32_t RTPSender::SendOutgoingData(
  ...
  uint32_t ret_val;
  if (audio_configured_) {
    TRACE_EVENT_ASYNC_STEP1("VoIP", "Audio", capture_timestamp,
    ...
    TRACE_EVENT_ASYNC_STEP1("VoIP", "Video", capture_time_ms,
                            "Send", "type", FrameTypeToString(frame_type));
    assert(frame_type != kAudioFrameSpeech && frame_type != kAudioFrameCN);
...
bool RTPSender::IsFecPacket(const uint8_t* buffer,
                            const AudioPacketHeader& header) const {
  if (!video_) {
    return false;
  }
  bool fec_enabled;
...
bool RTPSender::UpdateTransmissionTimeOffset(
    ...
    const AudioPacketHeader &rtp_header, const int64_t time_diff_ms) const {
  CriticalSectionScoped cs(send_critsect_);
  // Get id.
  uint8_t id = 0;
  if (rtp_header_extension_map_.GetId(kRtpExtensionTransmissionTimeOffset,
...
bool RTPSender::UpdateAudioLevel(uint8_t *rtp_packet,
                                 ...
                                 const AudioPacketHeader &rtp_header,
...
bool RTPSender::UpdateAbsoluteSendTime(
    uint8_t *rtp_packet, const uint16_t rtp_packet_length,
    const AudioPacketHeader &rtp_header, const int64_t now_ms) const {
  CriticalSectionScoped cs(send_critsect_);
...
void RTPSender::SetCSRCs(const uint32_t arr_of_csrc[kCsrcSize],
                         const uint8_t arr_length) {
  assert(arr_length <= kCsrcSize);
  CriticalSectionScoped cs(send_critsect_);
...
int32_t RTPSender::CSRCs(uint32_t arr_of_csrc[kCsrcSize]) const {
  assert(arr_of_csrc);
  CriticalSectionScoped cs(send_critsect_);
  for (int i = 0; i < num_csrcs_ && i < kCsrcSize; i++) {
