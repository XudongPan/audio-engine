...
#include "audio_engine/modules/audio_package/source/audio_pkg_receiver_impl.h"
...
// Get remote CSRC.
int32_t RtpReceiverImpl::CSRCs(uint32_t array_of_csrcs[kCsrcSize]) const {
  CriticalSectionScoped lock(critical_section_rtp_receiver_.get());

  assert(num_csrcs_ <= kCsrcSize);
...
int32_t RtpReceiverImpl::Energy(
    uint8_t array_of_energy[kCsrcSize]) const {
...
void RtpReceiverImpl::CheckSSRCChanged(const AudioPacketHeader& rtp_header) {
  bool new_ssrc = false;
    ...
    if (ssrc_ != rtp_header.ssrc ||
      ...
      if (ssrc_ != 0) {
        ...
        if (rtp_header.payloadType == last_received_payload_type) {
          ...
          assert(payload);
          payload_name[PAYLOAD_NAME_SIZE - 1] = 0;
          strncpy(payload_name, payload->name, PAYLOAD_NAME_SIZE - 1);
          if (payload->audio) {
...
void RtpReceiverImpl::CheckCSRC(const WebRtcRTPHeader& rtp_header) {
  ...
  uint32_t old_remote_csrc[kCsrcSize];
...

  {
    ...
    if ((num_csrcs > 0) && (num_csrcs <= kCsrcSize)) {
      // Copy new.
