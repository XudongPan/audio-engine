...
#ifdef MATLAB
#include "audio_engine/modules/audio_package/test/BWEStandAlone/MatlabPlot.h"
...
int32_t ModuleRtpRtcpImpl::CSRCs(
  uint32_t arr_of_csrc[kCsrcSize]) const {
  return rtp_sender_.CSRCs(arr_of_csrc);
}

int32_t ModuleRtpRtcpImpl::SetCSRCs(
    const uint32_t arr_of_csrc[kCsrcSize],
...
int32_t ModuleRtpRtcpImpl::SetRTCPVoIPMetrics(
  const RTCPVoIPMetric* webrtc_metric) {
  return  rtcp_sender_.SetRTCPVoIPMetrics(webrtc_metric);
}
