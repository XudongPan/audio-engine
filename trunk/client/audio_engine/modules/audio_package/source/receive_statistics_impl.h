...
class StreamStatisticianImpl : public StreamStatistician {
 public:
  ...
  virtual bool IsRetransmitOfOldPacket(const AudioPacketHeader& header,
                                       int min_rtt) const OVERRIDE;
  virtual bool IsPacketInOrder(uint16_t sequence_number) const OVERRIDE;

  void IncomingPacket(const AudioPacketHeader& rtp_header,
                      size_t bytes,
                      bool retransmitted);
 ...
 private:
  ...
  void UpdateJitter(const AudioPacketHeader& header,
                    uint32_t receive_time_secs,
                    uint32_t receive_time_frac);
  void UpdateCounters(const AudioPacketHeader& rtp_header,
                      size_t bytes,
                      bool retransmitted);
...
class ReceiveStatisticsImpl : public ReceiveStatistics,
 ...
 public:
  ...
  virtual void IncomingPacket(const AudioPacketHeader& header,
                              size_t bytes,
