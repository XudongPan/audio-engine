...
int32_t RTPPayloadRegistry::RegisterReceivePayload(
  ...
  *created_new_payload = false;
  ...
  if (ModuleRTPUtility::StringCompare(payload_name, "red", 3)) {
    ...
    payload->audio = false;
    strncpy(payload->name, payload_name, PAYLOAD_NAME_SIZE - 1);
...
bool RTPPayloadRegistry::IsRtx(const AudioPacketHeader& header) const {
  CriticalSectionScoped cs(crit_sect_.get());
  return IsRtxInternal(header);
}

bool RTPPayloadRegistry::IsRtxInternal(const AudioPacketHeader& header) const {
...
bool RTPPayloadRegistry::IsRed(const AudioPacketHeader& header) const {
  CriticalSectionScoped cs(crit_sect_.get());
  return red_payload_type_ == header.payloadType;
}

bool RTPPayloadRegistry::IsEncapsulated(const AudioPacketHeader& header) const {
...
void RTPPayloadRegistry::SetIncomingPayloadType(const AudioPacketHeader& header) {
  CriticalSectionScoped cs(crit_sect_.get());
...
class RTPPayloadAudioStrategy : public RTPPayloadStrategy {
 public:
  ...
  virtual ModuleRTPUtility::Payload* CreatePayloadType(
    ...
    payload->name[PAYLOAD_NAME_SIZE - 1] = 0;
    strncpy(payload->name, payloadName, PAYLOAD_NAME_SIZE - 1);
