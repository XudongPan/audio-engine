...
class RtpReceiver {
 public:
  ...
  virtual bool IncomingRtpPacket(const AudioPacketHeader& rtp_header,
  ...
  virtual int32_t CSRCs(uint32_t array_of_csrc[kCsrcSize]) const = 0;

  // Returns the current energy of the RTP stream received.
  virtual int32_t Energy(uint8_t array_of_energy[kCsrcSize]) const = 0;
};
}  // namespace VoIP
